...
const REFERENCE_SPEC_SHEET = {
  title: "ASUS TUF Gaming Z790-Plus WiFi — Service Reference",
  rows: [
    ["Model", "ASUS TUF Gaming Z790-Plus WiFi (Intel LGA1700, Z790)"],
    ["24-pin contact inspection", "Inspect main power connector pins for green/brown discoloration; clean corrosion before reseating — a corroded pin reads as connected but carries current poorly"],
    ["PSU fan service life", "Sleeve-bearing fans typically last 3-5 years continuous duty; a seized fan can trip the PSU's thermal protection and cause it to refuse to start or cut out under load"],
    ["Standoff torque/inspection", "Standoffs should sit flush against the case wall; vibration over years of service can back one out until it barely clears the board — re-torque snug, do not overtighten"],
    ["Diagnosis tip", "Symptoms that develop gradually and come and go (rather than a single hard failure) usually point to a worn or marginal connection, not one fully failed part"],
  ],
};

const DIAGNOSE_OPTIONS = [
  {
    id: "connectorCorrosion", correct: true,
    label: "24-pin power connector contacts are corroded",
    explain: "Correct — corroded contacts create a marginal, intermittent connection that can cause random power loss under vibration or thermal expansion.",
  },
  {
    id: "psuFanSeized", correct: true,
    label: "PSU fan has seized",
    explain: "Correct — a seized fan can trigger the PSU's thermal protection, causing it to refuse to start or cut out unpredictably.",
  },
  {
    id: "standoffBackedOut", correct: true,
    label: "A standoff has worked loose and is close to shorting the board",
    explain: "Correct — years of vibration can back a standoff out until it barely touches the board, causing intermittent shorts.",
  },
  {
    id: "ram", correct: false,
    label: "RAM stick is bad or improperly seated",
    explain: "Not the cause here — the symptom pattern points to power delivery, not memory.",
  },
  {
    id: "cmos", correct: false,
    label: "CMOS battery is dead",
    explain: "Not the cause here — a dead CMOS battery causes lost BIOS settings, not random power-offs during use.",
  },
];
...
const TASKS = [
  ...
  { id: "connectorCorrosion", label: "Clean the corroded 24-pin power connector contacts." },
  { id: "psuFanSeized", label: "Free/replace the seized PSU fan." },
  { id: "standoffBackedOut", label: "Reseat the standoff that's worked its way loose over time." },
...
export async function buildFPScene(container, { hintEl, toastEl, crosshairEl, taskListEl, wornItemsEl, modalOverlayEl, modalCardEl }) {
  ...
  const ticketTex = canvasTex(280, 360, (ctx, w, h) => {
    ...
    ctx.fillText("Years of continuous service, high-traffic location", 16, 82);

    ctx.font = "bold 10px Arial";
    ctx.fillText("REPORTED SYMPTOM", 16, 104);
    ctx.font = "10px Arial";
    const lines = [
      "Powers off randomly during use, or fails",
      "to start some mornings. Developed",
      "gradually over recent months, not a",
      "sudden failure.",
    ];
    ...
    const checks = [
      "24-pin connector corrosion", "PSU fan condition", "Case standoff position",
  ...
  // RAM is a red herring in this variant — properly seated, not part of
  // the fault, left as a plain non-interactive fixture so the scene still
  // reads as a complete build.
  ...
  // CMOS battery — a red herring in this variant, sitting fine in its
  // retention clip, left as a plain non-interactive fixture.
  ...
  const cmosBattery = new THREE.Mesh(
    ...
    new THREE.MeshStandardMaterial({ color: 0xc0c4ca, metalness: 0.7, roughness: 0.35 })
  );
  ...
  // Greenish/brownish patchy discoloration over the pin grid — corroded
  // contacts on the main power connector, this variant's headline fault.
  function makeCorrosionTexture() {
    return canvasTex(64, 64, (ctx, w, h) => {
      ...
      for (let x = 6; x < w - 6; x += 8) for (let y = 6; y < h - 6; y += 8) ctx.fillRect(x, y, 3, 3);
      const blotches = [
        ["rgba(94,110,64,0.55)", 0.42], ["rgba(120,92,52,0.5)", 0.3],
        ["rgba(80,120,90,0.45)", 0.34], ["rgba(102,78,46,0.5)", 0.26],
      ];
      blotches.forEach(([color, r], i) => {
        ctx.fillStyle = color;
        ctx.beginPath();
        ctx.ellipse(
          w * (0.2 + (i % 2) * 0.5), h * (0.25 + Math.floor(i / 2) * 0.45),
          w * r * 0.5, h * r * 0.3, i * 0.7, 0, Math.PI * 2
        );
        ctx.fill();
      });
    });
  }
  function makeCleanConnectorTexture() {
    return canvasTex(64, 64, (ctx, w, h) => {
      ctx.fillStyle = "#1c1e21";
      ctx.fillRect(0, 0, w, h);
  ...
  // The 24-pin main power connector is fully plugged in — the fault here is
  // corrosion on the contacts, not a loose connection, so it sits seated
  // (no raised gap) but wears a patchy corrosion texture until cleaned.
  ...
  const mainPower24 = new THREE.Mesh(
    ...
    new THREE.MeshStandardMaterial({ map: makeCorrosionTexture(), color: 0xffffff, roughness: 0.6 })
  );
  mainPower24.position.copy(mobo24Pos);
  mainPower24.userData = { interactable: true, kind: "connector", id: "connectorCorrosion" };
  ...
  // ---- PSU cooling fan, visually seized: blades frozen at an odd rest
  // angle with a dusty/grimy texture, mounted through the PSU's top grille.
  function makeGrimyFanBladeTexture() {
    return canvasTex(160, 160, (ctx, w, h) => {
      ctx.clearRect(0, 0, w, h);
      const cx = w / 2, cy = h / 2;
      ctx.fillStyle = "#8a8672";
      for (let i = 0; i < 5; i++) {
        const a = (i / 5) * Math.PI * 2;
        ctx.save();
        ctx.translate(cx, cy);
      ...
      // Grime/dust smudges caked onto the blade faces
      ctx.fillStyle = "rgba(70,58,34,0.4)";
      for (let i = 0; i < 10; i++) {
        ctx.beginPath();
        ctx.ellipse(Math.random() * w, Math.random() * h, 6 + Math.random() * 14, 4 + Math.random() * 8, Math.random() * Math.PI, 0, Math.PI * 2);
        ctx.fill();
      }
      ctx.fillStyle = "#1c1e21";
      ctx.beginPath();
  ...
  const psuFanPos = new THREE.Vector3(psuPos.x, psuPos.y + 0.044, psuPos.z);
  const seizedPsuFan = new THREE.Mesh(
    new THREE.CircleGeometry(0.045, 24),
    new THREE.MeshStandardMaterial({ map: makeGrimyFanBladeTexture(), roughness: 0.9, side: THREE.DoubleSide })
  );
  seizedPsuFan.rotation.x = -Math.PI / 2;
  // Odd rest angle — a seized fan doesn't stop lined up like a spinning one.
  seizedPsuFan.rotation.z = 0.4;
  seizedPsuFan.position.copy(psuFanPos);
  seizedPsuFan.userData = { interactable: true, kind: "psu-fan", id: "psuFanSeized" };
  internals.add(seizedPsuFan);

  const replacementPsuFan = buildArcticP12MaxFan(0.1);
  replacementPsuFan.rotation.x = -Math.PI / 2;
  replacementPsuFan.position.copy(psuFanPos);
  replacementPsuFan.visible = false;
  internals.add(replacementPsuFan);

  // ---- Standoff worked loose over years of vibration — protrudes further
  // toward the board than the others, close to touching it, until reseated.
  const standoffBoardLocal = new THREE.Vector3(-0.12, 0.05, 0.09);
  const standoffFlushPos = boardLocalToInternals(standoffBoardLocal);
  const standoffBackedOutPos = standoffFlushPos.clone();
  standoffBackedOutPos.x += 0.012;
  const standoffTex = canvasTex(32, 64, (ctx, w, h) => {
    ctx.fillStyle = "#9aa0a8";
    ctx.fillRect(0, 0, w, h);
  ...
  const standoff = new THREE.Mesh(
    ...
    new THREE.MeshStandardMaterial({ map: standoffTex, color: COLORS.standoff, metalness: 0.75, roughness: 0.35 })
  );
  standoff.rotation.z = Math.PI / 2;
  standoff.position.copy(standoffBackedOutPos);
  standoff.userData = { interactable: true, kind: "standoff", id: "standoffBackedOut" };
  ...
  function renderTaskList() {
    ...
    taskListEl.innerHTML = `<div class="tl-title">Workstation — Intermittent Power Issues</div><ul>${items}</ul>`;
  }
  ...
  function updateHint() {
    ...
    if (kind === "plug" && !taskState.unplug.done) text = `<kbd>E</kbd> Unplug from wall outlet`;
      ...
      else if (id === "connectorCorrosion") text = `<kbd>Click</kbd> Clean and reseat the 24-pin connector`;
    } else if (kind === "psu-fan" && !taskState.psuFanSeized.done) {
      text = taskState.diagnose.done ? `<kbd>Click</kbd> Free/replace the seized PSU fan` : `Diagnose the fault first`;
    } else if (kind === "standoff" && !taskState.standoffBackedOut.done) {
      text = taskState.diagnose.done ? `<kbd>Click</kbd> Screw the standoff back down flush` : `Diagnose the fault first`;
    }
  ...
  function repairsReady() {
    return taskState.connectorCorrosion.done && taskState.psuFanSeized.done && taskState.standoffBackedOut.done;
  }
  ...
  function fixConnector(id) {
    if (id === "connectorCorrosion") {
      // Clean-and-reseat: a quick pull-back-and-firm-push motion while the
      // corroded texture swaps out for clean contacts.
      let t = 0;
      const baseX = mainPower24.position.x;
      const anim = () => {
        t += 0.08;
        const tt = Math.min(1, t);
        mainPower24.position.x = baseX - Math.sin(tt * Math.PI) * 0.01;
        if (tt >= 1) {
          mainPower24.position.x = baseX;
          mainPower24.material.map = makeCleanConnectorTexture();
          mainPower24.material.needsUpdate = true;
          markDone("connectorCorrosion");
          toast("24-pin connector contacts cleaned and firmly reseated");
  ...
  function fixPsuFan() {
    seizedPsuFan.visible = false;
    replacementPsuFan.visible = true;
    markDone("psuFanSeized");
    toast("Seized PSU fan replaced — spinning freely");
  }

  function fixStandoff() {
    let t = 0;
    const startX = standoff.position.x;
    const anim = () => {
      t += 0.1;
      const tt = Math.min(1, t);
      standoff.position.x = startX + (standoffFlushPos.x - startX) * tt;
      standoff.rotation.x += 0.6;
      if (tt < 1) requestAnimationFrame(anim);
      else {
        standoff.position.copy(standoffFlushPos);
        markDone("standoffBackedOut");
        toast("Standoff screwed back down flush");
      }
    };
    anim();
  }
  ...
  function openManualModal() {
    const wrap = el("div");
    wrap.innerHTML = `<div class="mc-kicker">Reference</div><h2>${REFERENCE_SPEC_SHEET.title}</h2>`;
    const table = el("table", "spec-table");
    REFERENCE_SPEC_SHEET.rows.forEach(([label, value]) => {
      const tr = el("tr");
  ...
  function openDiagnoseModal() {
    ...
    wrap.innerHTML =
      ...
      `<p class="mc-body">Symptom: powers off randomly during use, or fails to start some mornings — developed gradually over recent months in a high-traffic office, not a sudden failure. Select every issue you can see.</p>`;
  ...
  function tryPrimaryAction() {
    ...
    if (kind === "screw" && !removedScrews.has(id)) {
      ...
      fixConnector(id);
    } else if (kind === "psu-fan" && taskState.diagnose.done && !taskState.psuFanSeized.done) {
      fixPsuFan();
    } else if (kind === "standoff" && taskState.diagnose.done && !taskState.standoffBackedOut.done) {
      fixStandoff();
  ...
  function animate() {
    ...
    if (replacementPsuFan.visible) {
      replacementPsuFan.userData.blade.rotateZ(dt * 6);
      replacementPsuFan.userData.wheel.rotateZ(dt * 6);
    }
