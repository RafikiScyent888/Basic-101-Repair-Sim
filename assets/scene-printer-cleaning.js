...
const CLEANING_SPEC_SHEET = {
  title: "HP LaserJet Pro M404dn — Internal Cleaning Service Notes",
  rows: [
    ["Power harness connector", "Feeds power from the inlet/PSU to the rest of the unit. It is friction-fit, not latched — a bumped or half-reseated connector during cleaning can look plugged in while carrying no continuity at all."],
    ["Fan cable routing clearance", "The cooling fan cable must be routed clear of the rear bracket and panel edge. If reassembled without checking clearance, the panel can pinch or shear the cable against the bracket."],
    ["Fuser thermal fuse", "A one-time protective fuse that trips (opens) if the unit is jolted or dropped during service, cutting power before any downstream damage occurs. It does not reset itself — it must be replaced."],
    ["Diagnosis tip", "Whenever a unit was opened for cleaning and comes back completely dead, check every connector and cable that was necessarily disturbed to get the cover off before suspecting an unrelated failure."],
  ],
};

const DIAGNOSE_OPTIONS = [
  {
    id: "harnessConn", correct: true,
    label: "Power harness connector was bumped loose",
    explain: "Correct — with the main power harness disconnected, no power reaches the rest of the unit at all, matching the complete dead-on-arrival symptom.",
  },
  {
    id: "fanCable", correct: true,
    label: "Cooling fan cable is pinched",
    explain: "Correct — a pinched fan cable can short or break the connection during reassembly, contributing to the unit refusing to start safely.",
  },
  {
    id: "thermalFuse", correct: true,
    label: "Fuser thermal fuse tripped",
    explain: "Correct — a jolt during cleaning/reassembly can trip this safety fuse, cutting power as a protective measure.",
  },
  {
    id: "toner", correct: false,
    label: "Toner cartridge is bad",
    explain: "Not the cause here — a bad cartridge affects print quality, not whether the unit powers on at all.",
  },
  {
    id: "cable", correct: false,
    label: "Power cable/strip is bad",
    explain: "Not the cause here — this ticket already confirms the power strip and cable are supplying power fine; the fault is internal, from what was disturbed during cleaning.",
  },
];
...
const TASKS = [
  ...
  { id: "harnessConn", label: "Reseat the power harness connector, bumped loose during cleaning." },
  { id: "fanCable", label: "Free the fan cable, pinched during reassembly." },
  { id: "thermalFuse", label: "Replace the fuser thermal fuse, tripped when bumped." },
...
export async function buildFPScene(container, { hintEl, toastEl, crosshairEl, taskListEl, wornItemsEl, modalOverlayEl, modalCardEl }) {
  ...
  const ticketTex = canvasTex(280, 360, (ctx, w, h) => {
    ...
    ctx.font = "bold 15px Arial";
    ctx.textAlign = "left";
    ctx.fillText("WORK ORDER  #2026-0731", 16, 28);
    ...
    ctx.fillText("Customer opened unit for internal", 16, 82);
    ctx.fillText("cleaning", 16, 94);
    ...
    const lines = [
      ...
      "sound — since the customer opened",
      "the unit to clean out dust.",
    ];
    ...
    const checks = [
      "Power harness connector", "Cooling fan cable", "Fuser thermal fuse",
      "Toner cartridge", "Power strip/cable",
    ];
    checks.forEach((c, i) => {
      const y = 228 + i * 20;
  ...
  // Decorative only in this scenario — the ticket's "toner cartridge" line
  // is a diagnose-modal distractor, not a physical fix task, so this prop
  // is not marked interactable.
  ...
  // Decorative only — not a physical fault in this scenario.

  // Rear power inlet (IEC) — exterior, decorative. The ticket's "power
  // strip/cable" line is a diagnose-modal distractor here: it's already
  // confirmed good, so this prop is not marked interactable.
  ...
  // Decorative only in this scenario — the PSU board itself isn't one of
  // the three faults, just background detail behind the real fault objects.
  ...
  fuse.userData = { interactable: true, kind: "connector", id: "thermalFuse" };
  internals.add(fuse);
  ...
  harnessGroup.userData = { interactable: true, kind: "connector", id: "harnessConn" };

  // Cooling fan cable — a small 2-pin connector pinched between the rear
  // mounting bracket and the panel edge during reassembly after cleaning.
  // Kept well clear of the fuse/harness/PSU footprints (x/y offsets below)
  // so hover/click never contends between them.
  const fanCableMat = new THREE.MeshStandardMaterial({ color: COLORS.connectorOff, roughness: 0.5 });
  const fanBracket = new THREE.Mesh(
    new THREE.BoxGeometry(0.05, 0.006, 0.024),
    new THREE.MeshStandardMaterial({ color: 0x6b7178, metalness: 0.5, roughness: 0.4 })
  );
  fanBracket.position.set(PW * 0.16, -0.012, -0.004);
  internals.add(fanBracket);
  const fanCableGroup = new THREE.Group();
  fanCableGroup.position.set(PW * 0.16, -0.018, 0.004);
  internals.add(fanCableGroup);
  // The connector sits wedged at an angle under the bracket edge — the
  // visible "pinch." Fixing it straightens and frees the connector.
  const fanConnector = new THREE.Mesh(new THREE.BoxGeometry(0.014, 0.009, 0.009), fanCableMat);
  fanConnector.rotation.z = 0.55;
  fanConnector.position.set(-0.006, -0.005, 0);
  fanCableGroup.add(fanConnector);
  const fanCableCurve = new THREE.CatmullRomCurve3([
    new THREE.Vector3(-0.018, -0.008, 0),
    new THREE.Vector3(-0.004, 0.002, 0.007),
    new THREE.Vector3(0.02, -0.01, 0.012),
  ]);
  const fanCableTube = new THREE.Mesh(
    new THREE.TubeGeometry(fanCableCurve, 20, 0.0025, 6, false),
    new THREE.MeshStandardMaterial({ color: 0x2b2f34 })
  );
  fanCableGroup.add(fanCableTube);
  fanCableGroup.userData = { interactable: true, kind: "connector", id: "fanCable" };
  ...
  // Reference manual / spec sheet (read-only, always available) — sits on
  // the front edge of the tool table, clear of the screwdriver/strap.
  ...
  manual.position.set(0, 0.77, 0.1);
  ...
  function renderTaskList() {
    ...
    const html = `<div class="tl-title">Printer Dead After Cleaning</div><ul>${TASKS.map((t) => {
  ...
  function updateHint() {
    ...
    else if (kind === "manual") text = `<kbd>E</kbd> Read internal cleaning service manual`;
    else if (kind === "connector" && !taskState[id]?.done) {
      if (!taskState.diagnose.done) text = `Diagnose the fault first`;
      else if ((id === "harnessConn" || id === "fanCable" || id === "thermalFuse") && !taskState.screws.done) text = `Open the back panel first`;
      else {
        const labels = {
          harnessConn: "Reseat power harness connector",
          fanCable: "Free pinched fan cable",
          thermalFuse: "Replace tripped thermal fuse",
        };
  ...
  function repairsReady() {
    return taskState.harnessConn.done && taskState.fanCable.done && taskState.thermalFuse.done;
  }
  ...
  function fixConnector(id) {
    if (id === "thermalFuse") {
      ...
      anim();
      markDone("thermalFuse");
      toast("Tripped thermal fuse replaced");
    } else if (id === "harnessConn") {
      ...
      anim();
      markDone("harnessConn");
      toast("Power harness connector reseated");
    } else if (id === "fanCable") {
      fanCableMat.color.setHex(COLORS.connectorOn);
      let t = 0;
      const startRot = fanConnector.rotation.z;
      const startPos = fanConnector.position.clone();
      const freePos = new THREE.Vector3(-0.006, 0.006, 0.01);
      const anim = () => {
        t += 0.06;
        const tt = Math.min(1, t);
        fanConnector.rotation.z = startRot * (1 - tt);
        fanConnector.position.lerpVectors(startPos, freePos, tt);
        if (t < 1) requestAnimationFrame(anim);
      };
      anim();
      markDone("fanCable");
      toast("Fan cable freed from the bracket and reseated");
    }
  }
  ...
  function openManualModal() {
    const wrap = el("div");
    wrap.innerHTML = `<div class="mc-kicker">Reference</div><h2>${CLEANING_SPEC_SHEET.title}</h2>`;
    const table = el("table", "spec-table");
    CLEANING_SPEC_SHEET.rows.forEach(([label, value]) => {
      const tr = el("tr");
  ...
  function openDiagnoseModal() {
    ...
    wrap.innerHTML =
      ...
      `<p class="mc-body">Symptom: completely dead — no lights, no sound, no response at all — since the customer opened the unit to clean out dust. Select every issue you can see.</p>`;
  ...
  function tryPrimaryAction() {
    ...
    if (kind === "screw" && !removedScrews.has(id)) {
      ...
      if ((id === "harnessConn" || id === "fanCable" || id === "thermalFuse") && !taskState.screws.done) return;
      fixConnector(id);
