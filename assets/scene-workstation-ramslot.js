...
const RAM_SPEC_SHEET = {
  title: "ASUS TUF Gaming Z790-Plus WiFi — Memory Configuration",
  rows: [
    ["Model", "ASUS TUF Gaming Z790-Plus WiFi (Intel LGA1700, Z790)"],
    ["Memory slots", "4x 288-pin DDR5 DIMM (Slots A1 / A2 / B1 / B2), max 128GB"],
    ["Slot contact wear", "Contact fingers on a DIMM slot can wear down after many insertion/removal cycles, eventually failing to make a reliable connection"],
    ["Seating technique", "Insert straight down with even pressure on both ends until both end clips snap closed — an angled or partial seat can still power on but fail POST or throw memory errors"],
    ["Contact oxidation", "Exposed gold-plated contacts can discolor and oxidize over time, especially in slots left empty or rarely reseated — this raises resistance the same way a worn slot does"],
    ["Diagnosis tip", "A long history of repeated reseating for unrelated repairs points toward slot or contact wear, not a single failed component"],
  ],
};

const DIAGNOSE_OPTIONS = [
  {
    id: "wornSlot", correct: true,
    label: "One DIMM slot's contacts are worn from repeated reseating",
    explain: "Correct — repeated insertion/removal over years can wear down a slot's contact fingers until they no longer make a reliable connection.",
  },
  {
    id: "crookedStick", correct: true,
    label: "A RAM stick is sitting crooked, not fully seated",
    explain: "Correct — a stick seated at an angle makes inconsistent contact and can cause intermittent memory errors or failed POSTs.",
  },
  {
    id: "oxidizedContacts", correct: true,
    label: "Another slot's contacts are oxidized",
    explain: "Correct — oxidized contacts increase resistance and can cause the same kind of intermittent memory errors described on the ticket.",
  },
  {
    id: "power24pin", correct: false,
    label: "24-pin main power connector is loose",
    explain: "Not the cause here — the specific symptom (memory errors) points to the RAM slots and sticks themselves, not general board power.",
  },
  {
    id: "cmos", correct: false,
    label: "CMOS battery is dead",
    explain: "Not the cause here — a dead CMOS battery causes lost BIOS settings, not memory errors tied to a specific reseating history.",
  },
];
...
const TASKS = [
  ...
  { id: "wornSlot", label: "Move the stick out of the worn slot and reseat it in a good one." },
  { id: "crookedStick", label: "Fully seat the stick that's sitting crooked." },
  { id: "oxidizedContacts", label: "Clean the oxidized contacts on the other RAM slot." },
...
// DIMM slot contact-finger strip texture, one variant per condition this
// scenario cares about: a clean healthy slot, a slot worn dull/scratched
// from repeated reseating, and a slot with greenish/brown oxidation over
// its gold plating. Shared by every DIMM slot on the board; only the
// condition passed in differs.
function makeSlotContactTex(condition) {
  return canvasTex(128, 32, (ctx, w, h) => {
    if (condition === "worn") {
      ctx.fillStyle = "#87806a";
      ctx.fillRect(0, 0, w, h);
      ctx.fillStyle = "rgba(0,0,0,0.28)";
      for (let i = 0; i < 30; i++) ctx.fillRect((i / 30) * w, 0, 1.2, h);
      for (let i = 0; i < 20; i++) {
        ctx.fillStyle = `rgba(35,32,24,${0.15 + Math.random() * 0.28})`;
        ctx.beginPath();
        ctx.ellipse(Math.random() * w, h / 2, 4 + Math.random() * 10, h * 0.42, 0, 0, Math.PI * 2);
        ctx.fill();
      }
      ctx.strokeStyle = "rgba(205,198,178,0.3)";
      ctx.lineWidth = 0.6;
      for (let i = 0; i < 22; i++) {
    ...
    } else if (condition === "oxidized") {
      ctx.fillStyle = "#ab8a3c";
      ctx.fillRect(0, 0, w, h);
      ctx.fillStyle = "rgba(0,0,0,0.25)";
      for (let i = 0; i < 30; i++) ctx.fillRect((i / 30) * w, 0, 1.2, h);
      for (let i = 0; i < 24; i++) {
        const g = 70 + Math.random() * 50;
        ctx.fillStyle = `rgba(${g * 0.6},${g},${g * 0.55},${0.3 + Math.random() * 0.3})`;
        ctx.beginPath();
        ctx.ellipse(Math.random() * w, h / 2, 4 + Math.random() * 12, h * 0.46, 0, 0, Math.PI * 2);
        ctx.fill();
      }
      for (let i = 0; i < 14; i++) {
        ctx.fillStyle = `rgba(95,58,28,${0.2 + Math.random() * 0.3})`;
        ctx.beginPath();
        ctx.ellipse(Math.random() * w, h / 2, 3 + Math.random() * 8, h * 0.35, 0, 0, Math.PI * 2);
        ctx.fill();
      }
    } else {
...
function buildTufMotherboard() {
  ...
  // Slot A1 (index 0) hosts the worn-slot fault, A2 (index 1) hosts the
  // crooked-stick fault in an otherwise-good slot, B1 (index 2) is the
  // oxidized-contacts fault left empty so the discoloration reads clearly,
  // and B2 (index 3) is a clean empty slot — the correct destination for
  // the stick pulled from the worn slot.
  const clipMat = new THREE.MeshStandardMaterial({ color: 0xc9cdd4, metalness: 0.6, roughness: 0.4 });
  const slotConditionByIndex = ["worn", "good", "oxidized", "good"];
  const slotMaterialsByCondition = {
    good: new THREE.MeshStandardMaterial({ map: makeSlotContactTex("good"), color: 0x2a2d32, metalness: 0.55, roughness: 0.35 }),
    worn: new THREE.MeshStandardMaterial({ map: makeSlotContactTex("worn"), color: 0x2a2d32, metalness: 0.25, roughness: 0.75 }),
    oxidized: new THREE.MeshStandardMaterial({ map: makeSlotContactTex("oxidized"), color: 0x2a2d32, metalness: 0.2, roughness: 0.8 }),
  };
  const dimmSlots = [];
  const dimmSlotMeshes = [];
  for (let i = 0; i < 4; i++) {
    const z = -0.06 + i * 0.018;
    const condition = slotConditionByIndex[i];
    const slot = new THREE.Mesh(new THREE.BoxGeometry(0.1, 0.008, 0.008), slotMaterialsByCondition[condition]);
    ...
    dimmSlotMeshes.push({ mesh: slot, condition });
  }
  group.userData.dimmSlots = dimmSlots;
  group.userData.dimmSlotMeshes = dimmSlotMeshes;
...
export async function buildFPScene(container, { hintEl, toastEl, crosshairEl, taskListEl, wornItemsEl, modalOverlayEl, modalCardEl }) {
  ...
  const ticketTex = canvasTex(280, 360, (ctx, w, h) => {
    ...
    ctx.font = "bold 15px Arial";
    ctx.textAlign = "left";
    ctx.fillText("WORK ORDER  #2026-0142", 16, 28);
    ...
    ctx.fillText("RAM reseated multiple times over", 16, 82);
    ctx.fillText("years of service history", 16, 94);

    ctx.font = "bold 10px Arial";
    ctx.fillText("REPORTED SYMPTOM", 16, 112);
    ctx.font = "10px Arial";
    const lines = [
      "Intermittent memory errors and",
      "occasional failure to POST. RAM has",
      "been reseated several times over the",
      "years for unrelated repairs.",
    ];
    ...
    const checks = [
      "Slot contact wear", "Stick seating/levelness", "Contact oxidation on other slots",
      "24-pin power connector", "CMOS battery",
    ];
    checks.forEach((c, i) => {
      const y = 216 + i * 20;
  ...
  const dimmSlotMeshes = boardGroup.userData.dimmSlotMeshes;
  const slotWornPos = boardLocalToInternals(dimmSlotsLocal[0]);
  const slotCrookedPos = boardLocalToInternals(dimmSlotsLocal[1]);
  const slotOxidizedPos = boardLocalToInternals(dimmSlotsLocal[2]);
  const slotGoodPos = boardLocalToInternals(dimmSlotsLocal[3]);

  // Empty, healthy slot (B2) — the correct destination once the stick is
  // pulled out of the worn slot.
  const slotGoodHotspot = new THREE.Object3D();
  slotGoodHotspot.position.copy(slotGoodPos);
  slotGoodHotspot.userData = { interactable: true, kind: "ram-slot", id: "slotGood" };
  internals.add(slotGoodHotspot);

  // Empty slot (B1) with visibly oxidized contacts — left with no stick in
  // it so the discoloration on its contact fingers reads clearly.
  const slotOxidizedHotspot = new THREE.Object3D();
  slotOxidizedHotspot.position.copy(slotOxidizedPos);
  slotOxidizedHotspot.userData = { interactable: true, kind: "oxidized-slot", id: "slotOxidized" };
  internals.add(slotOxidizedHotspot);

  // Worn slot (A1) — years of repeated reseating for unrelated repairs have
  // scuffed and dulled this slot's contact fingers. The stick sitting in it
  // looks fully seated (the slot itself is the fault, not the seating), so
  // the fix is pulling this stick and reseating it in the good slot (B2).
  const wornStick = buildKingstonFuryRam();
  wornStick.position.copy(slotWornPos);
  wornStick.userData = { interactable: true, kind: "ram-worn", id: "wornStick" };
  internals.add(wornStick);

  // Crooked stick (A2) — a good slot, but the stick was reseated at an
  // angle last time someone was in here and never sat all the way home.
  const crookedStick = buildKingstonFuryRam();
  crookedStick.position.copy(slotCrookedPos);
  crookedStick.position.y += 0.022;
  crookedStick.rotation.z = 0.32;
  crookedStick.userData = { interactable: true, kind: "ram-crooked", id: "crookedStick" };
  internals.add(crookedStick);
  ...
  // CMOS battery — present and working fine in this scenario. It's one of
  // the ticket's checklist items and a diagnose-modal distractor, but not
  // one of this variant's three RAM-focused faults, so it's left as a
  // plain decorative fixture (same treatment as the CPU power connector
  // above) rather than something the player can interact with.
  ...
  const cmosBattery = new THREE.Mesh(
    ...
    new THREE.MeshStandardMaterial({ color: 0xc7cbd1, metalness: 0.75, roughness: 0.25 })
  );
  ...
  // The 24-pin main power connector is properly seated and not part of
  // this fault — same decorative treatment as the CMOS battery above.
  ...
  let carriedPartKind = null; // "screw" | "ram-worn"
  const removedScrews = new Set();
  let wornStickPulled = false;
  let modalOpen = false;
  ...
  function renderTaskList() {
    ...
    taskListEl.innerHTML = `<div class="tl-title">Workstation — Memory Errors After Years of Reseating</div><ul>${items}</ul>`;
  }
  ...
  function updateHint() {
    ...
    else if (kind === "ram-worn" && !taskState.wornSlot.done) {
      if (!taskState.diagnose.done) text = `Diagnose the fault first`;
      else if (carriedPartMesh) text = `Hands full — place what you're carrying first`;
      else text = `<kbd>E</kbd> Pull stick from the worn slot`;
    } else if (kind === "ram-slot" && carriedPartKind === "ram-worn") {
      text = `<kbd>E</kbd> Insert stick into this slot`;
    } else if (kind === "ram-seat-worn" && !taskState.wornSlot.done) {
      text = `<kbd>Click</kbd> Push down evenly until it clicks into place`;
    } else if (kind === "ram-crooked" && !taskState.crookedStick.done) {
      text = taskState.diagnose.done ? `<kbd>Click</kbd> Straighten and fully seat the stick` : `Diagnose the fault first`;
    } else if (kind === "oxidized-slot" && !taskState.oxidizedContacts.done) {
      text = taskState.diagnose.done ? `<kbd>Click</kbd> Clean the oxidized contacts` : `Diagnose the fault first`;
    }
  ...
  function repairsReady() {
    return taskState.wornSlot.done && taskState.crookedStick.done && taskState.oxidizedContacts.done;
  }
  ...
  // Worn-slot fault: pull the stick out of the worn slot, carry it to the
  // good empty slot, then push it down to seat it — mirrors the pull /
  // insert / seat flow used elsewhere in this scenario.
  function pullWornStick() {
    wornStick.visible = false;
    wornStickPulled = true;
    const carried = buildKingstonFuryRam();
    partHandAnchor.add(carried);
    carriedPartMesh = carried;
    carriedPartKind = "ram-worn";
    toast("Pulled the stick from the worn slot");
  }

  function insertWornStick(slotId) {
    if (slotId !== "slotGood") {
      toast("That slot isn't a good target — check the manual");
      return;
    }
    partHandAnchor.remove(carriedPartMesh);
    carriedPartMesh = null;
    carriedPartKind = null;
    wornStick.visible = true;
    wornStick.position.copy(slotGoodPos);
    wornStick.position.y += 0.05;
    wornStick.userData = { interactable: true, kind: "ram-seat-worn", id: "wornStickSeat" };
    toast("Stick moved to a good slot — push down to seat it");
  }

  function seatWornStick() {
    let t = 0;
    const startY = wornStick.position.y;
    const anim = () => {
      t += 0.15;
      wornStick.position.y = startY + (slotGoodPos.y - startY) * Math.min(1, t);
      if (t < 1) requestAnimationFrame(anim);
      else {
        markDone("wornSlot");
        toast("Stick clicked into the good slot");
      }
    };
    anim();
  }

  // Crooked-stick fault: the slot itself is fine, so a single click
  // straightens the tilt and pushes the stick fully home.
  function seatCrookedStick() {
    let t = 0;
    const startY = crookedStick.position.y;
    const startRotZ = crookedStick.rotation.z;
    const anim = () => {
      t += 0.15;
      const tt = Math.min(1, t);
      crookedStick.position.y = startY + (slotCrookedPos.y - startY) * tt;
      crookedStick.rotation.z = startRotZ * (1 - tt);
      if (t < 1) requestAnimationFrame(anim);
      else {
        crookedStick.position.copy(slotCrookedPos);
        crookedStick.rotation.z = 0;
        markDone("crookedStick");
        toast("Stick straightened and fully seated");
      }
    };
    anim();
  }

  // Oxidized-contacts fault: swap the slot's contact-finger texture back
  // to a clean/healthy look.
  function cleanOxidizedContacts() {
    const slotInfo = dimmSlotMeshes[2];
    const mat = slotInfo.mesh.material;
    mat.map = makeSlotContactTex("good");
    mat.metalness = 0.55;
    mat.roughness = 0.35;
    mat.needsUpdate = true;
    markDone("oxidizedContacts");
    toast("Contacts cleaned");
  }
  ...
  function openManualModal() {
    const wrap = el("div");
    wrap.innerHTML = `<div class="mc-kicker">Reference</div><h2>${RAM_SPEC_SHEET.title}</h2>`;
    const table = el("table", "spec-table");
    RAM_SPEC_SHEET.rows.forEach(([label, value]) => {
      const tr = el("tr");
  ...
  function openDiagnoseModal() {
    ...
    wrap.innerHTML =
      ...
      `<p class="mc-body">Symptom: intermittent memory errors and occasional failure to POST. RAM has been reseated several times over the years for unrelated repairs. Select every issue you can see.</p>`;
  ...
  function tryPrimaryAction() {
    ...
    if (kind === "screw" && !removedScrews.has(id)) {
      ...
      seatWornStick();
    } else if (kind === "ram-crooked" && taskState.diagnose.done && !taskState.crookedStick.done) {
      seatCrookedStick();
    } else if (kind === "oxidized-slot" && taskState.diagnose.done && !taskState.oxidizedContacts.done) {
      cleanOxidizedContacts();
    }
  }
  ...
  function trySecondaryAction() {
    ...
    if (kind === "grab-tool" && heldTool !== id) pickUpTool(hovered);
      ...
      openDiagnoseModal();
    } else if (kind === "ram-worn" && !wornStickPulled && taskState.diagnose.done && !carriedPartMesh) {
      pullWornStick();
    } else if (kind === "ram-slot" && carriedPartKind === "ram-worn") {
      insertWornStick(id);
    }
