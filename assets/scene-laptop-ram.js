...
// This laptop only has 2 SO-DIMM slots (unlike the tower's 4-slot board), so
// the "wrong slot pairing" story is adapted: with only one slot to spare,
// the fault is that the two sticks got swapped end-for-end during the
// upgrade — the NEW stick landed in Slot 1 (primary) while the ORIGINAL
// stick ended up in Slot 2, when the manual requires the opposite.
const RAM_SPEC_SHEET = {
  title: "Razer Blade 18 — Memory Configuration",
  rows: [
    ["Model", "Razer Blade 18 (2026), Intel platform, dual-channel DDR5"],
    ["Memory slots", "2x 262-pin DDR5 SO-DIMM (Slot 1 / Slot 2), max 64GB"],
    ["Single-stick (1 DIMM) install", "Must use Slot 1 — the slot closest to the CPU, per QVL single-module guidance"],
    ["Upgrading to 2 sticks", "Keep the ORIGINAL module in Slot 1 and install the NEW module in Slot 2 — Slot 1 is wired to the channel the boot ROM trains first, and swapping modules there causes repeated retraining boot loops"],
    ["Seating", "Each stick must click fully into BOTH end latches — a stick that clicks on only one side is not seated"],
    ["Diagnosis tip", "A bent DIMM pin is usually caused by inserting a stick at an angle or with excess force; inspect closely before reseating."],
  ],
};

const DIAGNOSE_OPTIONS = [
  {
    id: "notLatched", correct: true,
    label: "One of the RAM sticks isn't fully latched in its slot",
    explain: "Correct — an unlatched stick can lose contact under the slightest vibration, causing an inconsistent boot.",
  },
  {
    id: "wrongSlot", correct: true,
    label: "New RAM stick is installed in the wrong slot",
    explain: "Correct — check the manual: this board has specific slot requirements that aren't being met.",
  },
  {
    id: "bentPin", correct: true,
    label: "A DIMM slot has a bent pin from a stick forced in at an angle",
    explain: "Correct — a bent contact pin can't reliably read that stick, which can single-handedly cause a boot loop.",
  },
  {
    id: "psu", correct: false,
    label: "AC adapter/charger is failing",
    explain: "Not the cause here — the timing (immediately after a RAM upgrade) and boot-loop pattern point squarely at memory, not power.",
  },
  {
    id: "cmos", correct: false,
    label: "CMOS/RTC battery is dead",
    explain: "Not the cause here — a dead RTC battery causes lost clock/BIOS settings over time, not a boot loop that started right after adding RAM.",
  },
];
...
const TASKS = [
  ...
  { id: "notLatched", label: "Fully latch the unseated RAM stick." },
  { id: "wrongSlot", label: "Move the new RAM stick to the correct slot." },
  { id: "bentPin", label: "Straighten/clear the bent pin in the DIMM slot." },
...
// Reused verbatim from the no-boot scenario's template — instantiated
// twice below (once for the original stick, once for the new stick).
...
export async function buildFPScene(container, { hintEl, toastEl, crosshairEl, taskListEl, wornItemsEl, modalOverlayEl, modalCardEl }) {
  ...
  const ticketTex = canvasTex(280, 360, (ctx, w, h) => {
    ...
    ctx.fillText("RAM upgraded from 1 to 2 sticks by customer", 16, 82);

    ctx.font = "bold 10px Arial";
    ctx.fillText("REPORTED SYMPTOM", 16, 104);
    ctx.font = "10px Arial";
    const lines = [
      "Boot-loops (power on/off/on",
      "repeatedly) or won't POST at all",
      "since a 1-to-2-stick RAM upgrade.",
    ];
    ...
    const checks = [
      "New stick latched", "Slot pairing", "Bent DIMM pin",
      "PSU/charger health", "CMOS/RTC battery",
    ];
    checks.forEach((c, i) => {
      const y = 206 + i * 20;
  ...
  const panelVentTex = canvasTex(128, 96, (ctx, w, h) => {
    ...
    ctx.font = "6px Arial";
    ctx.fillText("MODEL RZ09-0484  S/N 2026-0512-B", w * 0.5, h * 0.42);
  ...
  // Two SO-DIMM slots — Slot 1 (correct primary, closest to CPU) and Slot 2.
  ...
  const slot1Pos = slotPositions[0]; // correct/primary — closest to CPU
  const slot2Pos = slotPositions[1];

  // -------- RAM upgrade fault story --------
  // Only 2 slots exist on this board (unlike the desktop tower's 4), so the
  // "wrong slot" fault is a straight swap: the NEW stick ended up in Slot 1
  // (primary) while the ORIGINAL stick got bumped into Slot 2 during the
  // upgrade, and on top of that the original stick was never fully re-seated
  // (one latch clip popped). A third, independent fault — a bent contact pin
  // — sits in Slot 2 as well. All three faults are fixed independently and
  // in any order; each fault object's own userData.interactable flag is
  // switched off only once ITS OWN fix completes, so the two RAM sticks
  // never share state and a stale hover on one can't be triggered by fixing
  // the other (the exact stale-hover bug called out for this scenario).
  const RAM_TILT = THREE.MathUtils.degToRad(10);
  const RAM_POP = 0.014;

  // ramNew — the newly-added stick, currently (wrongly) in Slot 1.
  const ramNew = buildATechRam();
  ramNew.rotation.z = Math.PI / 2;
  ramNew.position.copy(slot1Pos);
  ramNew.userData = { interactable: true, kind: "ram-fault", id: "wrongSlot" };
  internals.add(ramNew);

  // ramOld — the original stick, currently in Slot 2, tilted/popped (not
  // fully latched). Tracked with its own live "current slot" position so the
  // notLatched fix and the wrongSlot swap can happen in either order without
  // fighting over the object's target transform.
  let ramOldSlotPos = slot2Pos.clone();
  const ramOld = buildATechRam();
  ramOld.rotation.z = Math.PI / 2;
  ramOld.position.copy(ramOldSlotPos);
  ramOld.position.y += RAM_POP;
  ramOld.rotation.x = RAM_TILT;
  ramOld.userData = { interactable: true, kind: "ram-fault", id: "notLatched" };
  internals.add(ramOld);

  // Bent contact pin in Slot 2 — a thin cylinder kicked out of the slot
  // channel at an angle instead of sitting flush. Small enough that it
  // exercises the same hover-fallback path as the RAM sticks themselves.
  const bentPinMat = new THREE.MeshStandardMaterial({ color: 0xd4af37, metalness: 0.85, roughness: 0.25 });
  const bentPin = new THREE.Mesh(new THREE.CylinderGeometry(0.0012, 0.0012, 0.01, 8), bentPinMat);
  bentPin.position.copy(slot2Pos);
  bentPin.position.x += 0.032;
  bentPin.position.y -= 0.003;
  const BENT_PIN_ANGLE = THREE.MathUtils.degToRad(38);
  bentPin.rotation.z = BENT_PIN_ANGLE;
  bentPin.userData = { interactable: true, kind: "ram-fault", id: "bentPin" };
  internals.add(bentPin);
  ...
  function renderTaskList() {
    ...
    const html = `<div class="tl-title">Laptop RAM Upgrade — Boot Loop</div><ul>${TASKS.map((t) => {
  ...
  function findLookedAtInteractable() {
    ...
    for (const obj of interactableRegistry) {
      ...
      // object whose one-time job is done (like a RAM stick once its own
      // fault is fixed) gets its flag turned off afterward, so that has to
      // be rechecked live here (and independently per-object) or a fixed
      // stick keeps catching hover/clicks meant for its still-broken
      // neighbor forever.
  ...
  function updateHint() {
    ...
    else if (kind === "manual") text = `<kbd>E</kbd> Read memory configuration manual`;
    ...
    else if (kind === "ram-fault" && taskState.diagnose.done && !taskState[id].done) {
      if (id === "wrongSlot") text = `<kbd>Click</kbd> Move this stick to the correct slot`;
      else if (id === "notLatched") text = `<kbd>Click</kbd> Push and latch the RAM stick`;
      else if (id === "bentPin") text = `<kbd>Click</kbd> Straighten the bent pin`;
    }
  ...
  function repairsReady() {
    return taskState.notLatched.done && taskState.wrongSlot.done && taskState.bentPin.done;
  }
  ...
  // Swap ramNew (Slot 1 → Slot 2) with ramOld (wherever it currently sits →
  // Slot 1). ramOld's own pop/tilt offset (its notLatched fault) travels
  // with it untouched — this fix only concerns which slot each stick is in,
  // not whether either one is seated flush yet.
  function fixWrongSlot() {
    let t = 0;
    const startNew = ramNew.position.clone();
    const startOld = ramOld.position.clone();
    const targetNew = slot2Pos.clone();
    const targetOld = slot1Pos.clone();
    targetOld.y += ramOld.position.y - ramOldSlotPos.y; // preserve current pop offset
    ramOldSlotPos = slot1Pos.clone();
    const anim = () => {
      t += 0.025;
      const tt = Math.min(1, t);
      const e = tt < 0.5 ? 2 * tt * tt : 1 - Math.pow(-2 * tt + 2, 2) / 2;
      const lift = Math.sin(tt * Math.PI) * 0.02;
      ramNew.position.lerpVectors(startNew, targetNew, e);
      ramNew.position.y += lift;
      ramOld.position.lerpVectors(startOld, targetOld, e);
      ramOld.position.y += lift;
      if (tt < 1) requestAnimationFrame(anim);
      else {
        ramNew.position.copy(targetNew);
        ramOld.position.copy(targetOld);
        ramNew.userData.interactable = false;
        markDone("wrongSlot");
        toast("Sticks swapped — new module now in Slot 2, original back in Slot 1");
      }
    };
    anim();
  }

  function fixNotLatched() {
    let t = 0;
    const startY = ramOld.position.y;
    const startRot = ramOld.rotation.x;
    const seatedY = ramOldSlotPos.y;
    const anim = () => {
      t += 0.1;
      const tt = Math.min(1, t);
      ramOld.position.y = startY + (seatedY - startY) * tt;
      ramOld.rotation.x = startRot * (1 - tt);
      if (tt < 1) requestAnimationFrame(anim);
      else {
        ramOld.position.y = seatedY;
        ramOld.rotation.x = 0;
        ramOld.userData.interactable = false;
        markDone("notLatched");
        toast("RAM stick clicked fully into both latches");
      }
    };
    anim();
  }
  ...
  function fixBentPin() {
    let t = 0;
    const startRot = bentPin.rotation.z;
    const anim = () => {
      t += 0.08;
      const tt = Math.min(1, t);
      bentPin.rotation.z = startRot * (1 - tt);
      if (tt < 1) requestAnimationFrame(anim);
      else {
        bentPin.rotation.z = 0;
        bentPin.userData.interactable = false;
        markDone("bentPin");
        toast("Bent pin straightened");
      }
    };
    anim();
  }
  ...
  function runTest() {
    ...
    const iv = setInterval(() => {
      ...
      if (blinks > 5) {
        ...
        if (success) {
          toast("POST successful — no more boot loop");
  ...
  function openDiagnoseModal() {
    ...
    wrap.innerHTML =
      ...
      `<p class="mc-body">Symptom: boot-loops (power on/off/on repeatedly) or won't POST at all, starting right after a 1-to-2-stick RAM upgrade. Select every issue you can see.</p>`;
  ...
  function tryPrimaryAction() {
    ...
    if (kind === "screw" && !removedScrews.has(id)) {
      ...
      if (id === "wrongSlot") fixWrongSlot();
      else if (id === "notLatched") fixNotLatched();
      else if (id === "bentPin") fixBentPin();
