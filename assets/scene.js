...
const RAM_SPEC_SHEET = {
  ...
  rows: [
    ...
    ["Single-stick (1 DIMM) install", "Must use Slot A2 — the second slot from the CPU, per QVL single-module guidance"],
    ["Dual-channel (2 DIMM) install", "Use Slot A2 + Slot B2"],
    ["Supported speeds", "DDR5-4800 to DDR5-5600 native; up to DDR5-7200 with XMP overclock"],
  ],
};
...
const DIAGNOSE_OPTIONS = [
  {
    ...
    explain: "Correct — check the manual: with only one stick installed, this board only reads Slot A2 at POST.",
  },
  {
    id: "cpuPower", correct: true,
    label: "8-pin CPU power connector is not fully seated",
    explain: "Correct — without full CPU power, the board can bring up fans/LEDs from the 24-pin alone but never posts.",
  },
  {
    id: "standoff", correct: true,
    label: "A stray standoff underneath the board is shorting it against the case",
    explain: "Correct — an extra standoff not aligned with a mounting hole shorts the board against the case.",
  },
  {
    id: "cmos", correct: false,
    label: "CMOS battery is dead",
    explain: "Not the cause here — a dead CMOS battery on a new build usually just resets BIOS settings, not fans/LEDs/POST entirely.",
  },
  {
    id: "psu", correct: false,
    label: "Power supply is DOA",
    explain: "Not the cause here — a dead PSU wouldn't spin the fans or light the case LEDs, and this build shows both.",
  },
];
...
const TASKS = [
  ...
  { id: "cpuPower", label: "Reseat the 8-pin CPU power connector." },
  { id: "standoff", label: "Remove the stray standoff shorting the board." },
...
export async function buildFPScene(container, { hintEl, toastEl, crosshairEl, taskListEl, wornItemsEl, modalOverlayEl, modalCardEl }) {
  ...
  const ticketTex = canvasTex(280, 360, (ctx, w, h) => {
    ...
    ctx.font = "bold 15px Arial";
    ctx.textAlign = "left";
    ctx.fillText("WORK ORDER  #2026-0091", 16, 28);
    ...
    ctx.fillText("Brand new self-build", 16, 82);
    ...
    const lines = [
      ...
      "lit — but no display output, no",
      "POST beep. Never reaches the OS.",
    ];
    ...
    const checks = [
      "RAM slot", "8-pin CPU power", "Stray standoff / short",
      "CMOS battery", "PSU",
    ];
    checks.forEach((c, i) => {
      const y = 206 + i * 20;
  ...
  cpuConn.userData = { interactable: true, kind: "connector", id: "cpuPower" };
  ...
  standoff.userData = { interactable: true, kind: "connector", id: "standoff" };
  ...
  function renderTaskList() {
    ...
    taskListEl.innerHTML = `<div class="tl-title">Tower Workstation Setup</div><ul>${items}</ul>`;
  }
  ...
  function updateHint() {
    ...
    if (kind === "plug" && !taskState.unplug.done) text = `<kbd>E</kbd> Unplug from wall outlet`;
      ...
      if (carriedPartKind === "ram") text = id === "slotB" ? `<kbd>E</kbd> Insert into Slot A2` : `Wrong slot — check the manual`;
      ...
      if (id === "cpuPower" && !taskState.cpuPower.done) text = `<kbd>Click</kbd> Reseat CPU power connector`;
      if (id === "standoff" && !taskState.standoff.done) text = `<kbd>Click</kbd> Remove shorting standoff`;
    }
  ...
  function repairsReady() {
    return taskState.ram.done && taskState.cpuPower.done && taskState.standoff.done;
  }
  ...
  function fixConnector(id) {
    if (id === "cpuPower") {
      ...
      cpuConn.position.y += 0.006;
      markDone("cpuPower");
      toast("CPU power connector reseated");
    } else if (id === "standoff") {
      let t = 0;
      const anim = () => {
        t += 0.1;
