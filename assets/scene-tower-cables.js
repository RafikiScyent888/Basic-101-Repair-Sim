...
const FRONT_PANEL_SPEC_SHEET = {
  title: "ASUS TUF Gaming Z790-Plus WiFi — Front Panel & Fan Headers",
  rows: [
    ["CPU_FAN header", "Must be connected for the board's thermal/fan-fail protection to see a valid signal"],
    ["Front panel header (F_PANEL)", "Power switch pins are labeled PWR_SW — check silkscreen pin 1 orientation before reconnecting"],
    ["SATA power", "Use a direct PSU cable run per drive; a loose connection can drop out under vibration"],
    ["Diagnosis tip", "Random shutdowns under load after cable work usually trace to a disturbed connector, not a failed component."],
  ],
};

const DIAGNOSE_OPTIONS = [
  {
    id: "cpuFan", correct: true,
    label: "CPU fan cable is unplugged from its header",
    explain: "Correct — with no CPU fan signal, the board trips its thermal/fan-fail protection and shuts the system down under load.",
  },
  {
    id: "pwrHeader", correct: true,
    label: "Front-panel power switch cable is on the wrong pins",
    explain: "Correct — misplaced power-switch pins can cause the system to behave as if the button is being pressed intermittently, triggering shutdowns/reboots.",
  },
  {
    id: "sataPower", correct: true,
    label: "Boot drive's SATA power cable is pinched loose",
    explain: "Correct — a marginal power connection to the drive can drop out under vibration or load, causing a hard shutdown or reboot.",
  },
  {
    id: "psu", correct: false,
    label: "Power supply is failing",
    explain: "Not the cause here — the PSU itself tests fine; the issues trace to specific cables disturbed during the cable-management pass.",
  },
  {
    id: "standoff", correct: false,
    label: "A stray standoff is shorting the board",
    explain: "Not the cause here — no short is present; this board mounts cleanly against its standoffs.",
  },
];
...
const TASKS = [
  ...
  { id: "cpuFan", label: "Plug the CPU fan cable back into its header." },
  { id: "pwrHeader", label: "Move the front-panel power switch cable to the correct pins." },
  { id: "sataPower", label: "Reseat the boot drive's SATA power cable." },
...
function buildTufMotherboard() {
  ...
  const fanCableMat = new THREE.MeshStandardMaterial({ color: 0x1c1e21, roughness: 0.8 });
  ...
  // Exposed so the caller can hide this default (already-seated) cable and
  // substitute a scene-specific dangling/unplugged version for a fault.
  group.userData.fanCable = fanCable;

  return group;
}

// A plain SATA SSD: flat metal-look box with a printed capacity label and a
// small power/data connector block on one edge — enough for a boot drive
// prop without needing to be photoreal.
function buildSimpleSsd() {
  const group = new THREE.Group();
  const labelTex = canvasTex(256, 160, (ctx, w, h) => {
    ctx.fillStyle = "#c7ccd2";
    ctx.fillRect(0, 0, w, h);
    for (let i = 0; i < 60; i++) {
    ...
    ctx.fillText("VELOCE", w / 2, h * 0.42);
    ctx.font = "16px Arial";
    ctx.fillText("SATA SSD — 1TB", w / 2, h * 0.62);
  });
  const body = new THREE.Mesh(
    new THREE.BoxGeometry(0.1, 0.007, 0.07),
    new THREE.MeshStandardMaterial({ color: 0xb8bec7, metalness: 0.55, roughness: 0.45 })
  );
  group.add(body);
  ...
  label.position.y = 0.0036;
  group.add(label);

  // Combined SATA data + power receptacle on the drive's rear edge
  const receptacle = new THREE.Mesh(
    new THREE.BoxGeometry(0.006, 0.012, 0.03),
    new THREE.MeshStandardMaterial({ color: 0x111214, roughness: 0.6 })
  );
  receptacle.position.set(-0.053, 0.002, 0.015);
  group.add(receptacle);
  group.userData.receptacleLocal = new THREE.Vector3(-0.056, 0.002, 0.015);
...
export async function buildFPScene(container, { hintEl, toastEl, crosshairEl, taskListEl, wornItemsEl, modalOverlayEl, modalCardEl }) {
  ...
  const ticketTex = canvasTex(280, 360, (ctx, w, h) => {
    ...
    ctx.font = "bold 15px Arial";
    ctx.textAlign = "left";
    ctx.fillText("WORK ORDER  #2026-0093", 16, 28);
    ...
    ctx.fillText("Cable management redone by customer", 16, 82);

    ctx.font = "bold 10px Arial";
    ctx.fillText("REPORTED SYMPTOM", 16, 104);
    ctx.font = "10px Arial";
    const lines = [
      "Boots normally, then shuts down or",
      "reboots unpredictably, especially",
      "under load, since a cable-",
      "management pass.",
    ];
    lines.forEach((line, i) => ctx.fillText(line, 16, 120 + i * 15));

    ctx.font = "bold 10px Arial";
    ctx.fillText("CHECK EVERY CAUSE YOU FIND:", 16, 188);
    const checks = [
      "CPU fan header", "Front-panel power switch pins",
      "Boot drive SATA power", "PSU health", "Stray standoff",
    ];
    checks.forEach((c, i) => {
      const y = 208 + i * 20;
  ...
  // RAM is correctly seated and not part of this scenario's fault story —
  // kept as a purely decorative, non-interactive prop for visual realism.
  ...
  // 8-pin CPU power target point — this cable stays fully connected in this
  // scenario (not a fault here), so it's just a plain target for addCable()
  // rather than a standalone interactive mesh.
  const cpuPowerTarget = new THREE.Vector3(chassisGroup.position.x + 0.06, chassisGroup.position.y + CH / 2 - 0.02, chassisGroup.position.z - 0.05);

  // Stray standoff is a diagnose-modal distractor only in this scenario —
  // present and correctly seated, not shorting anything, so it's decorative.
  const standoff = new THREE.Mesh(
    new THREE.CylinderGeometry(0.006, 0.006, 0.016, 8),
    new THREE.MeshStandardMaterial({ color: COLORS.standoff, metalness: 0.8, roughness: 0.3 })
  );
  standoff.position.set(chassisGroup.position.x + 0.06, chassisGroup.position.y - CH / 2 + 0.04, chassisGroup.position.z + 0.02);
  ...
  // A cable whose far end (anchor) stays fixed to a permanent component
  // while its near end tracks a moving plug — used for the two fault cables
  // that visibly drape/stretch as the player plugs them back in.
  function buildFollowCable(anchor, initialEnd, radius = 0.0035) {
    function curveFor(end) {
      const mid = anchor.clone().lerp(end, 0.5);
      mid.y -= 0.012;
      return new THREE.CatmullRomCurve3([anchor.clone(), mid, end.clone()]);
    }
    const mesh = new THREE.Mesh(new THREE.TubeGeometry(curveFor(initialEnd), 16, radius, 6, false), cableMat);
    internals.add(mesh);
    function update(end) {
      mesh.geometry.dispose();
      mesh.geometry = new THREE.TubeGeometry(curveFor(end), 16, radius, 6, false);
    }
    return { mesh, update };
  }
  ...
  addCable([
    psuTop,
    new THREE.Vector3(psuPos.x - 0.02, cpuPowerTarget.y - 0.05, chassisGroup.position.z - CD / 2 + 0.01),
    new THREE.Vector3(cpuPowerTarget.x - 0.01, cpuPowerTarget.y - 0.01, cpuPowerTarget.z),
  ], 0.009);

  // Fan header cables — front and rear case fans routed back to headers
  // along the board's edges. (These are the case intake/exhaust fans, a
  // separate thing from the CPU cooler fan fault below.)
  ...
  // ================= Fault 1: CPU fan cable unplugged =================
  // The board's own (already-seated) cooler fan cable is hidden and
  // replaced with a dangling cable + plug the player has to plug back in.
  boardGroup.userData.fanCable.visible = false;
  const fanHeaderLocal = new THREE.Vector3(-0.01, 0.012, -0.04);
  const fanHeaderPos = boardLocalToInternals(fanHeaderLocal);
  const fanHeaderBlock = new THREE.Mesh(
    new THREE.BoxGeometry(0.01, 0.008, 0.014),
    new THREE.MeshStandardMaterial({ map: connTex, color: 0x2b2f34, roughness: 0.5 })
  );
  fanHeaderBlock.position.copy(fanHeaderPos);
  internals.add(fanHeaderBlock);

  const fanCableAnchor = boardLocalToInternals(new THREE.Vector3(-0.03, 0.031, -0.02));
  const fanPlugRestPos = fanHeaderPos.clone();
  const fanPlugDanglePos = fanHeaderPos.clone().add(new THREE.Vector3(0.03, 0.035, 0.018));
  const fanPlug = new THREE.Mesh(
    new THREE.BoxGeometry(0.011, 0.015, 0.011),
    new THREE.MeshStandardMaterial({ map: connTex, color: COLORS.connectorOff, roughness: 0.5 })
  );
  fanPlug.position.copy(fanPlugDanglePos);
  fanPlug.userData = { interactable: true, kind: "connector", id: "cpuFan" };
  internals.add(fanPlug);
  const fanCableFollow = buildFollowCable(fanCableAnchor, fanPlugDanglePos, 0.003);

  // ========== Fault 2: front-panel power switch on wrong pins ==========
  const fpanelHeaderTex = canvasTex(96, 48, (ctx, w, h) => {
    ctx.fillStyle = "#111214";
    ctx.fillRect(0, 0, w, h);
    ctx.fillStyle = "#8a9098";
    ctx.textAlign = "center";
    ctx.font = "bold 9px Arial";
    ctx.fillText("F_PANEL", w / 2, h * 0.26);
    ctx.strokeStyle = "#4a4d52";
    ctx.lineWidth = 1;
    for (let row = 0; row < 2; row++) {
      for (let col = 0; col < 5; col++) {
        const x = w * 0.08 + col * w * 0.18, y = h * 0.48 + row * h * 0.28;
        ctx.strokeRect(x, y, w * 0.09, h * 0.18);
      }
    }
  });
  const fpanelHeaderLocal = new THREE.Vector3(0.11, 0.006, 0.1);
  const fpanelHeaderPos = boardLocalToInternals(fpanelHeaderLocal);
  const fpanelHeaderBlock = new THREE.Mesh(
    new THREE.BoxGeometry(0.024, 0.006, 0.013),
    new THREE.MeshStandardMaterial({ map: fpanelHeaderTex, roughness: 0.6 })
  );
  fpanelHeaderBlock.position.copy(fpanelHeaderPos);
  internals.add(fpanelHeaderBlock);

  // Wrong pins: shifted a couple of pin-pitches along the header's row.
  const fpanelWrongLocal = new THREE.Vector3(0.088, 0.006, 0.1);
  const fpanelWrongPos = boardLocalToInternals(fpanelWrongLocal);
  const pwrPlugGroup = new THREE.Group();
  pwrPlugGroup.position.copy(fpanelWrongPos);
  const pwrPlugBody = new THREE.Mesh(
    new THREE.BoxGeometry(0.009, 0.007, 0.014),
    new THREE.MeshStandardMaterial({ color: COLORS.connectorOff, roughness: 0.5 })
  );
  pwrPlugGroup.add(pwrPlugBody);
  // Short wire-bundle stub heading off toward the case's front-panel switch
  // — both ends belong to this one rigid group, so the whole thing can move
  // together without needing to regenerate any geometry.
  const pwrStubCurve = new THREE.CatmullRomCurve3([
    new THREE.Vector3(0, 0, 0.007),
    new THREE.Vector3(0.012, -0.012, 0.02),
    new THREE.Vector3(0.016, -0.022, 0.033),
  ]);
  const pwrStub = new THREE.Mesh(new THREE.TubeGeometry(pwrStubCurve, 10, 0.0025, 6, false), cableMat);
  pwrPlugGroup.add(pwrStub);
  pwrPlugGroup.userData = { interactable: true, kind: "connector", id: "pwrHeader" };
  internals.add(pwrPlugGroup);

  // ============ Fault 3: boot drive SATA power pinched loose ============
  const driveGroup = buildSimpleSsd();
  driveGroup.position.set(chassisGroup.position.x + 0.1, chassisGroup.position.y - CH / 2 + 0.07, chassisGroup.position.z - CD / 2 + 0.045);
  internals.add(driveGroup);
  const sataReceptaclePos = driveGroup.position.clone().add(driveGroup.userData.receptacleLocal);

  const sataAnchor = new THREE.Vector3(psuPos.x + 0.03, psuPos.y - 0.015, psuPos.z + 0.05);
  const sataLoosePos = sataReceptaclePos.clone().add(new THREE.Vector3(0.03, -0.014, 0.012));
  const sataPlug = new THREE.Mesh(
    new THREE.BoxGeometry(0.012, 0.014, 0.026),
    new THREE.MeshStandardMaterial({ color: COLORS.connectorOff, roughness: 0.5 })
  );
  sataPlug.position.copy(sataLoosePos);
  sataPlug.userData = { interactable: true, kind: "connector", id: "sataPower" };
  internals.add(sataPlug);
  const sataCableFollow = buildFollowCable(sataAnchor, sataLoosePos, 0.0032);
  ...
  [chassisGroup, powerBtnGroup, panelGroup, internals, led, ledGlow].forEach((obj) => towerPivot.attach(obj));
  ...
  function renderTaskList() {
    ...
    taskListEl.innerHTML = `<div class="tl-title">Cable Management Redo</div><ul>${items}</ul>`;
  }
  ...
  function updateHint() {
    ...
    else if (kind === "connector" && taskState.diagnose.done) {
      if (id === "cpuFan" && !taskState.cpuFan.done) text = `<kbd>Click</kbd> Plug the CPU fan cable into its header`;
      if (id === "pwrHeader" && !taskState.pwrHeader.done) text = `<kbd>Click</kbd> Move power switch cable to the correct pins`;
      if (id === "sataPower" && !taskState.sataPower.done) text = `<kbd>Click</kbd> Reseat the SATA power cable`;
    }
  ...
  function repairsReady() {
    return taskState.cpuFan.done && taskState.pwrHeader.done && taskState.sataPower.done;
  }
  ...
  function fixConnector(id) {
    if (id === "cpuFan") fixCpuFan();
    else if (id === "pwrHeader") fixPwrHeader();
    else if (id === "sataPower") fixSataPower();
  }

  function fixCpuFan() {
    let t = 0;
    const start = fanPlug.position.clone();
    const anim = () => {
      t += 0.09;
      const tt = Math.min(1, t);
      fanPlug.position.lerpVectors(start, fanPlugRestPos, tt);
      fanCableFollow.update(fanPlug.position);
      if (tt < 1) requestAnimationFrame(anim);
      else {
        fanPlug.material.color.setHex(COLORS.connectorOn);
        fanHeaderBlock.material.color.setHex(COLORS.connectorOn);
        markDone("cpuFan");
        toast("CPU fan cable plugged into CPU_FAN header");
      }
    };
    anim();
  }

  function fixPwrHeader() {
    let t = 0;
    const start = pwrPlugGroup.position.clone();
    const anim = () => {
      t += 0.09;
      const tt = Math.min(1, t);
      pwrPlugGroup.position.lerpVectors(start, fpanelHeaderPos, tt);
      if (tt < 1) requestAnimationFrame(anim);
      else {
        pwrPlugBody.material.color.setHex(COLORS.connectorOn);
        markDone("pwrHeader");
        toast("Power switch cable moved onto the correct PWR_SW pins");
      }
    };
    anim();
  }

  function fixSataPower() {
    let t = 0;
    const start = sataPlug.position.clone();
    const anim = () => {
      t += 0.1;
      const tt = Math.min(1, t);
      sataPlug.position.lerpVectors(start, sataReceptaclePos, tt);
      sataCableFollow.update(sataPlug.position);
      if (tt < 1) requestAnimationFrame(anim);
      else {
        // Quick snap-in bounce for a tactile "click" feel.
        let bt = 0;
        const bounce = () => {
          bt += 0.2;
          sataPlug.scale.setScalar(1 + Math.sin(Math.min(1, bt) * Math.PI) * 0.18);
          if (bt < 1) requestAnimationFrame(bounce);
          else sataPlug.scale.setScalar(1);
        };
        bounce();
        sataPlug.material.color.setHex(COLORS.connectorOn);
        markDone("sataPower");
        toast("SATA power cable fully seated");
      }
    };
    anim();
  }
  ...
  function openManualModal() {
    const wrap = el("div");
    wrap.innerHTML = `<div class="mc-kicker">Reference</div><h2>${FRONT_PANEL_SPEC_SHEET.title}</h2>`;
    const table = el("table", "spec-table");
    FRONT_PANEL_SPEC_SHEET.rows.forEach(([label, value]) => {
      const tr = el("tr");
  ...
  function openDiagnoseModal() {
    ...
    wrap.innerHTML =
      ...
      `<p class="mc-body">Symptom: boots fine, then shuts down or reboots unpredictably within minutes, especially under load, since a cable-management pass. Select every issue you can see.</p>`;
