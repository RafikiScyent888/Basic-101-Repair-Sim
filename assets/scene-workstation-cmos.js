...
const CMOS_SPEC_SHEET = {
  title: "ASUS TUF Gaming Z790-Plus WiFi — CMOS Battery & RTC Reference",
  rows: [
    ["Model", "ASUS TUF Gaming Z790-Plus WiFi (Intel LGA1700, Z790)"],
    ["CMOS battery", "CR2032 3V lithium coin cell, retained in a spring clip on the board edge"],
    ["Typical lifespan", "3-5 years continuous service; sooner if the unit is regularly fully unplugged"],
    ["Early symptoms", "Wrong system date/time after every power loss, BIOS settings (boot order, fan curves, XMP) silently reverting"],
    ["Late-stage symptom", "Once fully depleted, some boards fail to complete POST at all rather than just losing settings"],
    ["Terminal contact", "Corrosion or debris on the clip's contact spring can starve even a fresh battery of a reliable connection — check it whenever the battery is replaced"],
    ["Diagnosis tip", "A PSU fault produces sudden total failure with no history; weeks of drifting clock/settings beforehand points to the CMOS battery, not the power supply"],
  ],
};

const DIAGNOSE_OPTIONS = [
  {
    id: "cmosBattery", correct: true,
    label: "CMOS battery is dead",
    explain: "Correct — a dead CMOS battery explains the weeks of lost settings and wrong clock, and can ultimately prevent a clean POST once it fully dies.",
  },
  {
    id: "terminalCorrosion", correct: true,
    label: "Battery terminal contact is corroded",
    explain: "Correct — corrosion on the contact spring can prevent even a good battery from maintaining a reliable connection.",
  },
  {
    id: "ramReseat", correct: true,
    label: "RAM stick reverted to an unstable timing profile and needs reseating",
    explain: "Correct — after repeated BIOS resets from the dying battery, memory timing can revert to a setting this stick doesn't like until properly reseated.",
  },
  {
    id: "psu", correct: false,
    label: "Power supply is failing",
    explain: "Not the cause here — the weeks-long pattern of lost settings and wrong clock is a classic CMOS battery signature, not a PSU issue.",
  },
  {
    id: "power24pin", correct: false,
    label: "24-pin main power connector is loose",
    explain: "Not the cause here — nothing in this ticket points to a power-delivery issue; the symptom history points squarely at CMOS/battery.",
  },
];
...
const TASKS = [
  ...
  { id: "cmosBattery", label: "Replace the dead CMOS battery." },
  { id: "terminalCorrosion", label: "Clean the corroded battery terminal contact." },
  { id: "ramReseat", label: "Reseat the RAM stick that reverted to an unstable timing profile." },
...
export async function buildFPScene(container, { hintEl, toastEl, crosshairEl, taskListEl, wornItemsEl, modalOverlayEl, modalCardEl }) {
  ...
  const ticketTex = canvasTex(280, 360, (ctx, w, h) => {
    ...
    ctx.font = "bold 15px Arial";
    ctx.textAlign = "left";
    ctx.fillText("WORK ORDER  #2026-0261", 16, 28);
    ...
    ctx.fillText("In service several years, reporting", 16, 82);
    ctx.fillText("wrong date/time recently", 16, 94);

    ctx.font = "bold 10px Arial";
    ctx.fillText("REPORTED SYMPTOM", 16, 114);
    ctx.font = "10px Arial";
    const lines = [
      "Was losing BIOS settings and reporting",
      "the wrong date/time for weeks; today",
      "it won't POST at all.",
    ];
    ...
    const checks = [
      "CMOS battery voltage", "Battery terminal corrosion", "RAM seating/timing",
      "24-pin power connector", "PSU health",
    ];
    checks.forEach((c, i) => {
      const y = 216 + i * 20;
  ...
  const slotBHotspot = new THREE.Object3D();
  slotBHotspot.position.copy(slotBPos);
  slotBHotspot.userData = { interactable: true, kind: "ram-slot", id: "slotB" };
  internals.add(slotBHotspot);
  const slotAHotspot = new THREE.Object3D();
  slotAHotspot.position.copy(slotAPos);
  slotAHotspot.userData = { interactable: true, kind: "ram-slot", id: "slotA" };
  internals.add(slotAHotspot);

  // Years of thermal cycling and vibration have worked this stick partway
  // out of its slot — it's already in the correct slot (Slot A2 per the
  // manual), just not fully seated, so it starts raised rather than needing
  // to be pulled and moved to a different slot.
  const ramStick = buildKingstonFuryRam();
  ramStick.position.copy(slotBPos);
  ramStick.position.y += 0.05;
  ...
  // edge, dulled to look dead until replaced. This variant leans on this
  // battery hard: it's the headline fault, so it sits front-and-center on
  // the board's near edge, well within easy reach once the panel is open.
  ...
  const cmosBattery = new THREE.Mesh(
    ...
    new THREE.MeshStandardMaterial({ color: 0x6b6f75, metalness: 0.7, roughness: 0.35 })
  );
  ...
  cmosBattery.userData = { interactable: true, kind: "connector", id: "cmosBattery" };
  internals.add(cmosBattery);

  // Corrosion patch on the clip's contact spring — a small greenish/whitish
  // discoloration decal sitting right where the battery's negative face
  // touches the spring. Sized and placed to overlap the clip itself so it
  // reads as dirt on the actual contact point rather than a floating sticker.
  const corrosionTex = canvasTex(48, 48, (ctx, w, h) => {
    ctx.clearRect(0, 0, w, h);
    for (let i = 0; i < 26; i++) {
      ...
      ctx.fillStyle = hueMix > 0.5
        ? `rgba(120,150,90,${0.35 + Math.random() * 0.35})`
        : `rgba(210,215,190,${0.3 + Math.random() * 0.3})`;
      ctx.beginPath();
      ctx.ellipse(x, y, r, r * 0.7, Math.random() * Math.PI, 0, Math.PI * 2);
      ctx.fill();
    }
  });
  const corrosionPatch = new THREE.Mesh(
    new THREE.PlaneGeometry(0.012, 0.012),
    new THREE.MeshStandardMaterial({ map: corrosionTex, transparent: true, roughness: 0.9, side: THREE.DoubleSide })
  );
  corrosionPatch.rotation.x = -Math.PI / 2;
  corrosionPatch.position.set(cmosClip.position.x - 0.004, cmosClip.position.y + 0.0025, cmosClip.position.z - 0.005);
  corrosionPatch.userData = { interactable: true, kind: "connector", id: "terminalCorrosion" };
  internals.add(corrosionPatch);
  ...
  // The 24-pin main power connector is fully and properly seated in this
  // variant — the ticket lists it as a checklist item, but it's a distractor,
  // not one of this machine's actual findings, so it's plain decoration
  // (not interactable) sitting flush against the board header.
  ...
  let carriedPartKind = null; // "screw" | "ram"
  ...
  function renderTaskList() {
    ...
    taskListEl.innerHTML = `<div class="tl-title">Workstation — CMOS Battery Finally Dies</div><ul>${items}</ul>`;
  }
  ...
  function updateHint() {
    ...
    else if (kind === "ram-seat" && !taskState.ramReseat.done) {
      text = taskState.diagnose.done ? `<kbd>Click</kbd> Push down evenly until it clicks into place` : `Diagnose the fault first`;
    } else if (kind === "connector" && !taskState[id]?.done) {
      if (!taskState.diagnose.done) text = `Diagnose the fault first`;
      else if (id === "cmosBattery") text = `<kbd>Click</kbd> Replace CMOS battery`;
      else if (id === "terminalCorrosion") text = `<kbd>Click</kbd> Clean corroded terminal contact`;
    }
  ...
  function repairsReady() {
    return taskState.ramReseat.done && taskState.terminalCorrosion.done && taskState.cmosBattery.done;
  }
  ...
  function insertRam(slotId) {
    ...
    ramStick.position.y += 0.05;
    ramStick.userData = { interactable: true, kind: "ram-seat", id: "ramSeatConfirm" };
    toast("RAM inserted into Slot A2 — push down to seat it");
  }
  ...
  function seatRam() {
    ...
    const anim = () => {
      t += 0.15;
      ramStick.position.y = startY + (slotBPos.y - startY) * Math.min(1, t);
      if (t < 1) requestAnimationFrame(anim);
      else {
        markDone("ramReseat");
        toast("RAM clicked into place");
      }
    };
    anim();
  }
  ...
  function fixConnector(id) {
    if (id === "terminalCorrosion") {
      corrosionPatch.visible = false;
      markDone("terminalCorrosion");
      toast("Terminal contact cleaned");
    } else if (id === "cmosBattery") {
      cmosBattery.material.color.setHex(0xd8dadd);
      let t = 0;
      const anim = () => {
        t += 0.15;
        cmosBattery.position.y = cmosClip.position.y + 0.003 + Math.sin(t * Math.PI) * 0.006;
        if (t < 1) requestAnimationFrame(anim);
        else cmosBattery.position.y = cmosClip.position.y + 0.003;
      };
      anim();
      markDone("cmosBattery");
      toast("CMOS battery replaced");
    }
  }
  ...
  function openDiagnoseModal() {
    ...
    wrap.innerHTML =
      ...
      `<p class="mc-body">Symptom: worked fine yesterday, no POST this morning, no beep, no display. Fans spin normally, case LEDs lit. No known changes were made. Select every issue you can see.</p>`;
