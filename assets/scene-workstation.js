...
const RAM_SPEC_SHEET = {
  ...
  rows: [
    ...
    ["Currently installed", "1x DIMM in Slot A2 (correct slot for single-stick per QVL guidance)"],
...
const TASKS = [
  ...
  { id: "ram", label: "Reseat the RAM stick that's worked loose." },
  { id: "power24", label: "Reseat the 24-pin main power connector." },
  { id: "cmos", label: "Replace the dead CMOS battery." },
...
export async function buildFPScene(container, { hintEl, toastEl, crosshairEl, taskListEl, wornItemsEl, modalOverlayEl, modalCardEl }) {
  ...
  const ticketTex = canvasTex(280, 360, (ctx, w, h) => {
    ...
    ctx.fillText("In service 3 years", 16, 82);
    ...
    const lines = [
      ...
      "no POST, no beep, no display. Fans",
      "spin normally, case LEDs lit. No known",
    ...
    const checks = [
      "CMOS battery", "RAM seating", "24-pin main power",
      "GPU failure", "BIOS corruption",
    ];
    checks.forEach((c, i) => {
      const y = 216 + i * 20;
  ...
  // edge, dulled to look dead until replaced.
  ...
  cmosBattery.userData = { interactable: true, kind: "connector", id: "cmos" };
  ...
  // The 24-pin main power connector has worked partway out of its header —
  // it starts raised, with the cable run ending at that raised gap, and
  // seats flush once reseated.
  const power24GapPos = mobo24Pos.clone().add(new THREE.Vector3(0, 0.014, 0));
  addCable([psuTop, new THREE.Vector3(psuPos.x + 0.04, psuPos.y + 0.05, psuPos.z + 0.02), power24GapPos], 0.012, false);
  ...
  mainPower24.position.copy(power24GapPos);
  mainPower24.userData = { interactable: true, kind: "connector", id: "power24" };
  ...
  function renderTaskList() {
    ...
    taskListEl.innerHTML = `<div class="tl-title">Workstation Suddenly Won't POST</div><ul>${items}</ul>`;
  }
  ...
  function updateHint() {
    ...
    if (kind === "plug" && !taskState.unplug.done) text = `<kbd>E</kbd> Unplug from wall outlet`;
      ...
      else if (id === "power24") text = `<kbd>Click</kbd> Reseat 24-pin main power connector`;
      else if (id === "cmos") text = `<kbd>Click</kbd> Replace CMOS battery`;
    }
  ...
  function repairsReady() {
    return taskState.ram.done && taskState.power24.done && taskState.cmos.done;
  }
  ...
  function fixConnector(id) {
    if (id === "power24") {
      mainPower24.material.color.setHex(COLORS.connectorOn);
      let t = 0;
      const startY = mainPower24.position.y;
      const anim = () => {
        t += 0.12;
        mainPower24.position.y = startY - (startY - mobo24Pos.y) * Math.min(1, t);
        if (t < 1) requestAnimationFrame(anim);
        else mainPower24.position.y = mobo24Pos.y;
      };
      anim();
      markDone("power24");
      toast("24-pin main power connector reseated");
    } else if (id === "cmos") {
