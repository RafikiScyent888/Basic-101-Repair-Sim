...
const TONER_SPEC_SHEET = {
  title: "HP LaserJet Pro M404dn — Toner Cartridge Service Notes",
  rows: [
    ["Cartridge", "HP 58A (CF258A) — genuine or compatible drum/toner unit"],
    ["Installation step 1", "Unbag the cartridge and gently rock it side-to-side to redistribute toner before installing"],
    ["Installation step 2 — seal", "Pull the orange/yellow sealing tab straight out in one motion. The drum will not release toner onto the page until this seal is fully removed."],
    ["Installation step 3 — seating", "Align the cartridge's rails with the bay guides and slide it in until it clicks fully seated. A cartridge resting on the rails but not clicked in will not transfer toner."],
    ["Chip contact alignment", "The gold contact pad on the cartridge must sit flush and square against the printer's reader pins. A twisted or rotated contact reads inconsistently (or not at all), even though the print cycle itself runs normally."],
    ["Diagnosis tip", "Rule out toner seating, the seal, and chip contact alignment before suspecting the fuser or paper path — a normal print cycle with completely blank output almost always traces back to toner never reaching the page, not a downstream hardware fault."],
  ],
};

const DIAGNOSE_OPTIONS = [
  {
    id: "tonerSeated", correct: true,
    label: "Toner cartridge isn't fully seated",
    explain: "Correct — an unseated cartridge can't properly transfer toner onto the drum, resulting in blank output even though the print cycle runs.",
  },
  {
    id: "sealRemoved", correct: true,
    label: "Protective seal was never removed from the new cartridge",
    explain: "Correct — new cartridges ship with a pull-tab seal over the drum; left in place, no toner can reach the page at all.",
  },
  {
    id: "chipContact", correct: true,
    label: "Toner cartridge's chip contact is misaligned",
    explain: "Correct — this printer reads the cartridge's chip to calibrate output; a misaligned contact can cause it to silently print with no toner applied.",
  },
  {
    id: "fuser", correct: false,
    label: "Fuser assembly has failed",
    explain: "Not the cause here — a failed fuser usually causes smearing or toner that rubs off, not completely blank pages; this points to toner not reaching the page at all.",
  },
  {
    id: "paperPath", correct: false,
    label: "Paper path is jammed or misaligned",
    explain: "Not the cause here — the ticket confirms pages feed through fine and come out blank, not jammed.",
  },
];
...
const TASKS = [
  ...
  { id: "tonerSeated", label: "Fully seat the toner cartridge." },
  { id: "sealRemoved", label: "Remove the drum's protective seal/tape." },
  { id: "chipContact", label: "Realign the toner cartridge's chip contact." },
...
export async function buildFPScene(container, { hintEl, toastEl, crosshairEl, taskListEl, wornItemsEl, modalOverlayEl, modalCardEl }) {
  ...
  const ticketTex = canvasTex(280, 360, (ctx, w, h) => {
    ...
    ctx.fillText("Toner cartridge replaced by customer", 16, 82);

    ctx.font = "bold 10px Arial";
    ctx.fillText("REPORTED SYMPTOM", 16, 104);
    ctx.font = "10px Arial";
    const lines = [
      "Powers on and runs a normal print",
      "cycle, but every page comes out",
      "completely blank, since a customer",
      "toner cartridge swap.",
    ];
    ...
    const checks = [
      "Toner cartridge seated", "Protective seal/tape removed",
      "Toner chip contact alignment", "Fuser assembly", "Paper path",
    ];
    checks.forEach((c, i) => {
      const y = 226 + i * 20;
  ...
  // Purely decorative in this variant — the fault is inside, not the paper path.
  ...
  // (the fault here is toner, not power delivery).
  ...
  function renderTaskList() {
    ...
    const html = `<div class="tl-title">Printer — Prints Blank After Toner Swap</div><ul>${TASKS.map((t) => {
