...
const COLORS = {
  ...
  ledGood: 0x22c55e,
  ledBad: 0xef4444,
  cablePinchBad: 0xef4444,
  cablePinchGood: 0x2b2f34,
  hingeBracket: 0x6b7178,
};

const DISPLAY_SPEC_SHEET = {
  title: "Razer Blade 18 — Display Cable & Hinge Service Notes",
  rows: [
    ["Model", "Razer Blade 18 (2026), eDP display cable routed through the left hinge"],
    ["Cable routing", "Must clear the hinge barrel by at least 3mm; route along the molded channel, never draped across the metal edge"],
    ["Connector seating", "ZIF connector must click flush at the board; the locking flap should lie flat once latched"],
    ["Pinched / kinked cable", "A cable crossing the hinge's sharp edge can crush or sever its internal traces — total signal loss even though the board still posts normally"],
    ["Diagnosis tip", "Rule out the cable/connector (cheap, common after a DIY screen/hinge job) before condemning the display panel itself (expensive, rare)"],
  ],
};

const DIAGNOSE_OPTIONS = [
  {
    id: "cableUnseated", correct: true,
    label: "Display cable isn't seated at the motherboard connector",
    explain: "Correct — a disconnected display cable means no video signal can leave the board at all, even though everything else powers up fine.",
  },
  {
    id: "cablePinched", correct: true,
    label: "Display cable is pinched at the hinge",
    explain: "Correct — routing the cable across a sharp hinge edge during reassembly can crush or sever its internal traces.",
  },
  {
    id: "strayScrew", correct: true,
    label: "A stray screw is shorting the board",
    explain: "Correct — a loose screw left in from the repair can short components and disrupt normal operation.",
  },
  {
    id: "ram", correct: false,
    label: "RAM stick is bad or improperly seated",
    explain: "Not the cause here — a bad display doesn't stem from memory, and everything else about the boot behaves normally.",
  },
  {
    id: "panel", correct: false,
    label: "The display panel itself is dead",
    explain: "Not the cause here — jumping straight to 'replace the panel' skips the much more likely, and much cheaper, cable/connector issues introduced during the repair.",
  },
];
...
const TASKS = [
  ...
  { id: "cableUnseated", label: "Reseat the display cable connector at the motherboard." },
  { id: "cablePinched", label: "Reroute the display cable clear of the hinge pinch point." },
...
export async function buildFPScene(container, { hintEl, toastEl, crosshairEl, taskListEl, wornItemsEl, modalOverlayEl, modalCardEl }) {
  ...
  const ticketTex = canvasTex(280, 360, (ctx, w, h) => {
    ...
    ctx.font = "bold 15px Arial";
    ctx.textAlign = "left";
    ctx.fillText("WORK ORDER  #2026-0522", 16, 28);
    ...
    ctx.fillText("screen/hinge repair attempted", 16, 82);
    ...
    ctx.font = "10px Arial";
    const lines = [
      "Powers on, fan spins, backlight/",
      "LED active, but display stays",
      "black since a customer screen/",
      "hinge repair.",
    ];
    ...
    const checks = [
      "Display cable seated at board", "Display cable pinched at hinge",
      "Stray reassembly screws", "RAM seated", "Display panel itself",
    ];
    checks.forEach((c, i) => {
      const y = 226 + i * 20;
  ...
  const panelVentTex = canvasTex(128, 96, (ctx, w, h) => {
    ...
    ctx.font = "6px Arial";
    ctx.fillText("MODEL RZ09-0484  S/N 2026-0522-A", w * 0.5, h * 0.42);
  ...
  // Two SO-DIMM slots — Slot 1 (correct, closest to CPU) and Slot 2
  ...
  const slot1Pos = slotPositions[0]; // closest to CPU
  const slot2Pos = slotPositions[1];

  // RAM is a distractor in this scenario — properly seated, in the correct
  // slot, and not interactable. It's here so the diagnose modal's "RAM
  // stick is bad or improperly seated" option has something plausible to
  // point at, not because it's actually part of this fault.
  const ramStick = buildATechRam();
  ramStick.rotation.z = Math.PI / 2;
  ramStick.position.copy(slot1Pos);
  internals.add(ramStick);

  // Display cable connector — a ZIF (zero-insertion-force) socket with a
  // small hinged locking flap, exactly how a real eDP cable clips in.
  ...
  const displayConn = new THREE.Mesh(
  ...
  displayConn.position.set(0.02, -0.003, LD * 0.4);
  displayConn.userData = { interactable: true, kind: "connector", id: "cableUnseated" };
  internals.add(displayConn);

  const latchGroup = new THREE.Group();
  latchGroup.position.set(0, 0.003, -0.006);
  displayConn.add(latchGroup);
  const latchFlap = new THREE.Mesh(
    new THREE.BoxGeometry(0.02, 0.0015, 0.006),
    new THREE.MeshStandardMaterial({ color: 0xc9cdd4, metalness: 0.5, roughness: 0.4 })
  );
  latchFlap.position.z = 0.003;
  latchGroup.add(latchFlap);
  latchGroup.rotation.x = -1.1; // disconnected: flap flipped open

  // Display cable run from the ZIF connector back toward the hinge — a
  // simple decorative tube tying the connector fault and the hinge-pinch
  // fault together as the same physical cable, per the customer's botched
  // screen/hinge repair.
  const cableRunCurve = new THREE.CatmullRomCurve3([
    new THREE.Vector3(0.02, -0.001, LD * 0.4),
    new THREE.Vector3(0.035, 0.001, LD * 0.44),
    new THREE.Vector3(0.045, 0.003, LD * 0.465),
  ]);
  const cableRun = new THREE.Mesh(
    new THREE.TubeGeometry(cableRunCurve, 16, 0.0015, 6, false),
    new THREE.MeshStandardMaterial({ color: COLORS.cablePinchGood, roughness: 0.7 })
  );
  internals.add(cableRun);

  // Hinge-area prop — a small metal bracket standing in for the hinge
  // knuckle the display cable has to clear on its way up into the lid.
  const hingeBracket = new THREE.Mesh(
    new THREE.BoxGeometry(0.014, 0.014, 0.05),
    new THREE.MeshStandardMaterial({ color: COLORS.hingeBracket, metalness: 0.7, roughness: 0.3 })
  );
  hingeBracket.rotation.y = 0.3;
  hingeBracket.position.set(0.07, 0.001, LD * 0.46);
  internals.add(hingeBracket);

  // Pinched/kinked section of that same cable, draped straight across the
  // hinge bracket's sharp corner instead of clearing it — the "screen
  // repair gone wrong" fault. Rotated and lifted off-path so it visibly
  // reads as crimped rather than following the clean cable run.
  const kinkMarker = new THREE.Mesh(
    new THREE.BoxGeometry(0.016, 0.004, 0.006),
    new THREE.MeshStandardMaterial({ color: COLORS.cablePinchBad, roughness: 0.55 })
  );
  kinkMarker.position.set(0.048, 0.006, LD * 0.465);
  kinkMarker.rotation.z = 0.7;
  kinkMarker.userData = { interactable: true, kind: "connector", id: "cablePinched" };
  internals.add(kinkMarker);

  // Stray screw shorting the board against the chassis
  ...
  standoff.position.set(-LW * 0.1, -0.0065, LD * 0.21);
  standoff.userData = { interactable: true, kind: "connector", id: "strayScrew" };
  ...
  function renderTaskList() {
    ...
    const html = `<div class="tl-title">Screen/Hinge Repair, Still Black</div><ul>${TASKS.map((t) => {
  ...
  function updateHint() {
    ...
    if (kind === "plug" && !taskState.unplug.done) text = `<kbd>E</kbd> Unplug AC adapter from wall outlet`;
    ...
    else if (kind === "manual") text = `<kbd>E</kbd> Read display cable & hinge service notes`;
      ...
      if (id === "cableUnseated" && !taskState.cableUnseated.done) text = `<kbd>Click</kbd> Reconnect display cable`;
      if (id === "cablePinched" && !taskState.cablePinched.done) text = `<kbd>Click</kbd> Reroute pinched cable clear of hinge`;
      if (id === "strayScrew" && !taskState.strayScrew.done) text = `<kbd>Click</kbd> Remove shorting screw`;
    }
  ...
  function repairsReady() {
    return taskState.cableUnseated.done && taskState.cablePinched.done && taskState.strayScrew.done;
  }
  ...
  function fixConnector(id) {
    if (id === "cableUnseated") {
      displayConn.material.color.setHex(COLORS.connectorOn);
      displayConn.position.y += 0.004;
      let t = 0;
      const startRot = latchGroup.rotation.x;
      const anim = () => {
        t += 0.1;
        latchGroup.rotation.x = startRot * (1 - Math.min(1, t));
        if (t < 1) requestAnimationFrame(anim);
        else latchGroup.rotation.x = 0;
      };
      anim();
      markDone("cableUnseated");
      toast("Display cable reconnected");
    } else if (id === "cablePinched") {
      let t = 0;
      const startRot = kinkMarker.rotation.z;
      const startY = kinkMarker.position.y;
      const anim = () => {
        t += 0.1;
        const tt = Math.min(1, t);
        kinkMarker.rotation.z = startRot * (1 - tt);
        kinkMarker.position.y = startY - 0.004 * tt;
        if (t < 1) requestAnimationFrame(anim);
        else kinkMarker.material.color.setHex(COLORS.cablePinchGood);
      };
      anim();
      markDone("cablePinched");
      toast("Cable rerouted clear of the hinge");
  ...
  function runTest() {
    ...
    const iv = setInterval(() => {
      ...
      if (blinks > 5) {
        ...
        if (success) {
          toast("Display back online — POST successful");
  ...
  function openManualModal() {
    const wrap = el("div");
    wrap.innerHTML = `<div class="mc-kicker">Reference</div><h2>${DISPLAY_SPEC_SHEET.title}</h2>`;
    const table = el("table", "spec-table");
    DISPLAY_SPEC_SHEET.rows.forEach(([label, value]) => {
      const tr = el("tr");
  ...
  function openDiagnoseModal() {
    ...
    wrap.innerHTML =
      ...
      `<p class="mc-body">Symptom: powers on (fan spins, keyboard backlight on), screen stays completely black since a customer screen/hinge repair attempt. Select every issue you can see.</p>`;
