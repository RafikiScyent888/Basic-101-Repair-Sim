...
const TRAY_SPEC_SHEET = {
  title: "HP LaserJet Pro M404dn — Duplex / Tray 2 Install Notes",
  rows: [
    ["Duplex unit", "Slides into the rear accessory bay on rails and must click fully home. A module resting on its rails but not locked in can shift during a job and throw off the paper path."],
    ["Tray sensor cable", "The small connector at the tray's mounting point must be firmly seated. Without it, the printer can't read paper level or position in the new tray."],
    ["Feed roller (Tray 2)", "Must sit fully engaged against the paper stack. A roller left lifted in its shipping position can't grip and advance paper reliably."],
    ["Diagnosis tip", "When jams or misfeeds start right after installing new hardware, check that new hardware first — the duplex lock, sensor cable, and feed roller — before suspecting toner or the fuser, which didn't change."],
  ],
};

const DIAGNOSE_OPTIONS = [
  {
    id: "duplexLocked", correct: true,
    label: "Duplex unit isn't fully locked into place",
    explain: "Correct — a loose duplex module can shift during a print job and throw off the paper path, causing jams on double-sided output.",
  },
  {
    id: "traySensor", correct: true,
    label: "Tray sensor cable is disconnected",
    explain: "Correct — without this sensor's signal, the printer can't correctly detect paper level or position in the new tray, causing misfeeds.",
  },
  {
    id: "feedRoller", correct: true,
    label: "Feed roller for the new tray isn't engaged",
    explain: "Correct — a disengaged roller can't reliably grip and advance paper from that tray, causing jams almost every time it's used.",
  },
  {
    id: "toner", correct: false,
    label: "Toner cartridge is bad",
    explain: "Not the cause here — the symptom is paper jamming/misfeeding, a paper-path issue, not a print-quality issue.",
  },
  {
    id: "fuser", correct: false,
    label: "Fuser assembly has failed",
    explain: "Not the cause here — the jams trace specifically to the newly-installed tray and duplex hardware, not the existing fuser.",
  },
];
...
const TASKS = [
  ...
  { id: "duplexLocked", label: "Fully lock the duplex unit into place." },
  { id: "traySensor", label: "Reconnect the tray sensor cable." },
  { id: "feedRoller", label: "Re-engage the feed roller for the new tray." },
...
export async function buildFPScene(container, { hintEl, toastEl, crosshairEl, taskListEl, wornItemsEl, modalOverlayEl, modalCardEl }) {
  ...
  const ticketTex = canvasTex(280, 360, (ctx, w, h) => {
    ...
    ctx.font = "bold 15px Arial";
    ctx.textAlign = "left";
    ctx.fillText("WORK ORDER  #2026-0774", 16, 28);
    ...
    ctx.fillText("Second tray + duplex unit", 16, 82);
    ctx.fillText("installed by customer", 16, 94);

    ctx.font = "bold 10px Arial";
    ctx.fillText("REPORTED SYMPTOM", 16, 116);
    ctx.font = "10px Arial";
    const lines = [
      "Jams or misfeeds on nearly every",
      "job since a customer-installed",
      "second tray and duplex unit.",
    ];
    lines.forEach((line, i) => ctx.fillText(line, 16, 132 + i * 15));

    ctx.font = "bold 10px Arial";
    ctx.fillText("CHECK EVERY CAUSE YOU FIND:", 16, 210);
    const checks = [
      "Duplex unit locked in", "Tray sensor cable", "Feed roller engagement",
      "Toner cartridge", "Fuser assembly",
    ];
    checks.forEach((c, i) => {
      const y = 226 + i * 20;
  ...
  // LCD, front-loading paper tray, and a rear panel over the control board.
  ...
  // behind it (matched to the back panel) for the decorative control board
  // to sit in — a solid full-depth shell would hide it once the panel comes off.
  ...
  // Purely decorative in this variant — the fault is the newly-added tray
  // hardware, not toner (a diagnose distractor, not a physical fix task).
  printerGroup.add(toner);

  // Front-loading paper tray, protruding slightly with a paper stack visible.
  // Also decorative here — the existing front tray/paper path isn't the fault.
  ...
  // Rear power inlet (IEC) — exterior, decorative here (the fault isn't power delivery).
  ...
  // Purely decorative in this variant — the customer-installed tray/duplex
  // hardware (the actual faults) is a separate, always-visible accessory
  // bolted onto the outside of the rear-bottom of the case (see below), not
  // hidden inside this service cavity. Opening the panel still shows the
  // existing control board so the step isn't a dead end, it just isn't
  // where any of this scenario's faults live.
  ...
  const boardTex = canvasTex(256, 256, (ctx, w, h) => {
    ...
    ctx.font = "10px Arial";
    ctx.fillText("RM2-5399 FORMATTER BOARD — OK", w * 0.06, h * 0.94);
  });
  const internalBoard = new THREE.Mesh(
    new THREE.BoxGeometry(PW * 0.5, PH * 0.45, 0.006),
    new THREE.MeshStandardMaterial({ map: boardTex, roughness: 0.75 })
  );
  internalBoard.position.set(0, 0.02, -0.008);
  internals.add(internalBoard);

  // -------- Duplex/second-tray assembly (rear-bottom exterior) --------
  // Bolted onto the OUTSIDE of the printer's rear-bottom edge, like a real
  // clip-on duplexer/second-tray accessory — always visible (not gated
  // behind the back panel), and reachable from the same walk-around-the-
  // desk vantage point already used for the back panel screws.
  const DUPLEX_UNSEATED_Z = -PD / 2 - 0.05;
  const DUPLEX_SEATED_Z = -PD / 2 - 0.015;
  const duplexGroup = new THREE.Group();
  duplexGroup.position.set(0, -PH / 2 + 0.04, DUPLEX_UNSEATED_Z);
  printerGroup.add(duplexGroup);
  const duplexBodyTex = canvasTex(128, 64, (ctx, w, h) => {
    ...
    ctx.font = "8px Arial";
    ctx.textAlign = "left";
    ctx.fillText("DUPLEX / TRAY 2 UNIT", w * 0.05, h * 0.25);
  });
  const duplexBody = new THREE.Mesh(
    new THREE.BoxGeometry(PW * 0.55, 0.05, 0.035),
    new THREE.MeshStandardMaterial({ map: duplexBodyTex, roughness: 0.6 })
  );
  duplexGroup.add(duplexBody);
  const duplexLatchMat = new THREE.MeshStandardMaterial({ color: COLORS.connectorOff, roughness: 0.5 });
  const duplexLatch = new THREE.Mesh(new THREE.BoxGeometry(0.018, 0.012, 0.012), duplexLatchMat);
  duplexLatch.position.set(PW * 0.24, 0.006, 0.02);
  duplexGroup.add(duplexLatch);
  duplexGroup.userData = { interactable: true, kind: "connector", id: "duplexLocked" };

  // Tray sensor cable connector — near the new tray's mounting point,
  // shown unplugged (plug hangs clear of its socket).
  const TRAY_SENSOR_UNPLUGGED_Z = -PD / 2 - 0.045;
  const TRAY_SENSOR_PLUGGED_Z = -PD / 2 - 0.012;
  const traySensorGroup = new THREE.Group();
  traySensorGroup.position.set(PW * 0.42, -PH / 2 + 0.09, 0);
  printerGroup.add(traySensorGroup);
  const traySensorSocket = new THREE.Mesh(
    new THREE.BoxGeometry(0.02, 0.016, 0.012),
    new THREE.MeshStandardMaterial({ color: 0x2b2f34, roughness: 0.6 })
  );
  traySensorSocket.position.set(0, 0, TRAY_SENSOR_PLUGGED_Z);
  traySensorGroup.add(traySensorSocket);
  const traySensorMat = new THREE.MeshStandardMaterial({ color: COLORS.connectorOff, roughness: 0.5 });
  const traySensorPlug = new THREE.Mesh(new THREE.BoxGeometry(0.016, 0.012, 0.02), traySensorMat);
  traySensorPlug.position.set(0, 0, TRAY_SENSOR_UNPLUGGED_Z);
  traySensorGroup.add(traySensorPlug);
  const traySensorCable = new THREE.Mesh(
    new THREE.CylinderGeometry(0.003, 0.003, 0.03, 8),
    new THREE.MeshStandardMaterial({ color: 0x2b2f34 })
  );
  traySensorCable.rotation.x = Math.PI / 2;
  traySensorCable.position.set(0, 0, TRAY_SENSOR_UNPLUGGED_Z - 0.02);
  traySensorGroup.add(traySensorCable);
  traySensorGroup.userData = { interactable: true, kind: "connector", id: "traySensor" };

  // Feed roller for the new tray's paper path — shown lifted clear of its
  // normal contact position (disengaged), drops into place when fixed.
  const FEED_ROLLER_ENGAGED_Y = -PH / 2 + 0.05;
  const FEED_ROLLER_DISENGAGED_Y = FEED_ROLLER_ENGAGED_Y + 0.025;
  const feedRollerGroup = new THREE.Group();
  feedRollerGroup.position.set(-PW * 0.42, FEED_ROLLER_DISENGAGED_Y, -PD / 2 - 0.02);
  printerGroup.add(feedRollerGroup);
  const feedRollerMat = new THREE.MeshStandardMaterial({ color: 0x2b2f34, roughness: 0.65 });
  const feedRoller = new THREE.Mesh(new THREE.CylinderGeometry(0.014, 0.014, 0.05, 14), feedRollerMat);
  feedRoller.rotation.z = Math.PI / 2;
  feedRollerGroup.add(feedRoller);
  const feedRollerAxle = new THREE.Mesh(
    new THREE.CylinderGeometry(0.004, 0.004, 0.07, 8),
    new THREE.MeshStandardMaterial({ color: 0x9aa0a8, metalness: 0.7, roughness: 0.3 })
  );
  feedRollerAxle.rotation.z = Math.PI / 2;
  feedRollerGroup.add(feedRollerAxle);
  feedRollerGroup.userData = { interactable: true, kind: "connector", id: "feedRoller" };
  ...
  function renderTaskList() {
    ...
    const html = `<div class="tl-title">Printer — Duplexer/Tray Fault</div><ul>${TASKS.map((t) => {
  ...
  function updateHint() {
    ...
    else if (kind === "manual") text = `<kbd>E</kbd> Read duplex/tray install manual`;
    else if (kind === "connector" && !taskState[id]?.done) {
      if (!taskState.diagnose.done) text = `Diagnose the fault first`;
      else {
        const labels = {
          duplexLocked: "Lock the duplex unit into place",
          traySensor: "Reconnect the tray sensor cable",
          feedRoller: "Re-engage the feed roller for the new tray",
        };
  ...
  function repairsReady() {
    return taskState.duplexLocked.done && taskState.traySensor.done && taskState.feedRoller.done;
  }
  ...
  function fixConnector(id) {
    if (id === "duplexLocked") {
      let t = 0;
      const startZ = duplexGroup.position.z;
      const anim = () => {
        t += 0.08;
        duplexGroup.position.z = THREE.MathUtils.lerp(startZ, DUPLEX_SEATED_Z, Math.min(1, t));
        if (t < 1) requestAnimationFrame(anim);
        else { duplexGroup.position.z = DUPLEX_SEATED_Z; duplexLatchMat.color.setHex(COLORS.connectorOn); }
      };
      anim();
      markDone("duplexLocked");
      toast("Duplex unit slid fully in and locked");
    } else if (id === "traySensor") {
      traySensorMat.color.setHex(COLORS.connectorOn);
      let t = 0;
      const startZ = traySensorPlug.position.z;
      const anim = () => {
        t += 0.1;
        traySensorPlug.position.z = THREE.MathUtils.lerp(startZ, TRAY_SENSOR_PLUGGED_Z, Math.min(1, t));
        if (t < 1) requestAnimationFrame(anim);
        else traySensorPlug.position.z = TRAY_SENSOR_PLUGGED_Z;
      };
      anim();
      markDone("traySensor");
      toast("Tray sensor cable reconnected");
    } else if (id === "feedRoller") {
      let t = 0;
      const startY = feedRollerGroup.position.y;
      const anim = () => {
        t += 0.1;
        feedRollerGroup.position.y = THREE.MathUtils.lerp(startY, FEED_ROLLER_ENGAGED_Y, Math.min(1, t));
        if (t < 1) requestAnimationFrame(anim);
        else feedRollerGroup.position.y = FEED_ROLLER_ENGAGED_Y;
      };
      anim();
      markDone("feedRoller");
      toast("Feed roller re-engaged for the new tray");
    }
  }
  ...
  function openManualModal() {
    const wrap = el("div");
    wrap.innerHTML = `<div class="mc-kicker">Reference</div><h2>${TRAY_SPEC_SHEET.title}</h2>`;
    const table = el("table", "spec-table");
    TRAY_SPEC_SHEET.rows.forEach(([label, value]) => {
      const tr = el("tr");
  ...
  function openDiagnoseModal() {
    ...
    wrap.innerHTML =
      ...
      `<p class="mc-body">Symptom: jams or misfeeds on nearly every job since a customer-installed second tray and duplex unit. Select every issue you can see.</p>`;
