...
const STORAGE_SPEC_SHEET = {
  title: "ASUS TUF Gaming Z790-Plus WiFi — Storage Configuration",
  rows: [
    ["M.2_1 slot", "Requires the mounting screw to keep the drive flush; an unseated drive can disconnect under vibration"],
    ["SATA ports", "Each drive needs BOTH a data cable (to the board) and a power cable (to the PSU) — either alone is not enough"],
    ["Boot drive", "Windows install lives on the original SATA drive, not the new M.2"],
    ["Diagnosis tip", "POST completing but stopping before the OS loads usually points to the boot drive itself being disconnected or unrecognized."],
  ],
};

const DIAGNOSE_OPTIONS = [
  {
    id: "m2Seated", correct: true,
    label: "New M.2 drive isn't fully seated / its mounting screw is missing",
    explain: "Correct — a raised M.2 drive can flex out of contact and destabilize the bus, and it's also plainly not secured down.",
  },
  {
    id: "sataData", correct: true,
    label: "Boot drive's SATA data cable is unplugged",
    explain: "Correct — without a data connection, the board can't see the Windows drive at all, so it never gets past POST.",
  },
  {
    id: "sataPower", correct: true,
    label: "Boot drive's SATA power cable is unplugged",
    explain: "Correct — the boot drive needs both a data and a power connection; missing either makes it invisible to the system.",
  },
  {
    id: "ram", correct: false,
    label: "RAM stick is bad or improperly seated",
    explain: "Not the cause here — POST completes cleanly, which confirms memory is fine.",
  },
  {
    id: "cmos", correct: false,
    label: "CMOS battery is dead",
    explain: "Not the cause here — a dead CMOS battery wouldn't produce a clean POST; it would usually just reset BIOS settings.",
  },
];
...
const TASKS = [
  ...
  { id: "m2Seated", label: "Fully seat and screw down the new M.2 drive." },
  { id: "sataData", label: "Reconnect the boot drive's SATA data cable." },
  { id: "sataPower", label: "Reconnect the boot drive's SATA power cable." },
...
export async function buildFPScene(container, { hintEl, toastEl, crosshairEl, taskListEl, wornItemsEl, modalOverlayEl, modalCardEl }) {
  ...
  const ticketTex = canvasTex(280, 360, (ctx, w, h) => {
    ...
    ctx.font = "bold 15px Arial";
    ctx.textAlign = "left";
    ctx.fillText("WORK ORDER  #2026-0094", 16, 28);
    ...
    ctx.fillText("Second M.2 drive installed by customer", 16, 82);

    ctx.font = "bold 10px Arial";
    ctx.fillText("REPORTED SYMPTOM", 16, 104);
    ctx.font = "10px Arial";
    const lines = [
      "POSTs normally, but stops before",
      "Windows loads, since a second M.2",
      "drive was installed.",
    ];
    ...
    const checks = [
      "New M.2 seated/screwed down", "Boot drive SATA data cable",
      "Boot drive SATA power cable", "RAM seated", "CMOS battery",
    ];
    checks.forEach((c, i) => {
      const y = 206 + i * 20;
  ...
  // RAM isn't a fault in this scenario (it's a diagnose-modal distractor) —
  // shown already correctly seated in Slot A2, purely decorative.
  ...
  // 8-pin CPU power is already properly seated here (not a fault this time)
  // — kept purely as a decorative, already-connected component.
  ...
  // -------- Fault 1: second M.2 drive, resting at an angle, unscrewed --------
  // Positioned on the board via boardLocalToInternals so it visually sits
  // right where a real M.2_1 slot would be — clear in X of the DIMM slots
  // and chipset (which only extend to local x ~0.07) and clear in Z of the
  // CPU socket, between the socket and the chipset badge.
  const m2SlotLocal = new THREE.Vector3(0.11, 0.011, -0.02); // connector end (nearest CPU)
  const m2ScrewLocal = new THREE.Vector3(0.11, 0.011, 0.05); // far mounting-hole end
  const m2SlotPos = boardLocalToInternals(m2SlotLocal);
  const m2ScrewPos = boardLocalToInternals(m2ScrewLocal);
  const m2Length = m2SlotPos.distanceTo(m2ScrewPos);

  // Physical M.2 key-slot connector (static, doesn't move)
  const m2SlotConn = new THREE.Mesh(
    new THREE.BoxGeometry(0.02, 0.003, 0.006),
    new THREE.MeshStandardMaterial({ color: 0x1a1c1f, roughness: 0.6 })
  );
  m2SlotConn.position.copy(m2SlotPos);
  internals.add(m2SlotConn);
  // Standoff post at the far mounting hole (static)
  const m2StandoffPost = new THREE.Mesh(
    new THREE.CylinderGeometry(0.0035, 0.0035, 0.006, 8),
    new THREE.MeshStandardMaterial({ color: COLORS.standoff, metalness: 0.7, roughness: 0.35 })
  );
  m2StandoffPost.position.copy(m2ScrewPos);
  internals.add(m2StandoffPost);

  const m2LabelTex = canvasTex(64, 256, (ctx, w, h) => {
    ...
    ctx.textAlign = "center";
    ctx.font = "bold 15px Arial";
    ctx.fillText("NVMe SSD", 0, -4);
    ctx.font = "9px Arial";
    ctx.fillStyle = "#cfe3f7";
    ctx.fillText("1TB M.2 2280", 0, 12);
    ctx.restore();
  });
  const m2StickMesh = new THREE.Mesh(
    new THREE.BoxGeometry(0.012, 0.001, m2Length - 0.004),
    new THREE.MeshStandardMaterial({ map: m2LabelTex, roughness: 0.6 })
  );
  m2StickMesh.position.z = m2Length / 2;
  const m2Screw = new THREE.Mesh(
    new THREE.CylinderGeometry(0.0032, 0.0032, 0.004, 10),
    new THREE.MeshStandardMaterial({ map: screwHeadTex, metalness: 0.75, roughness: 0.3 })
  );
  m2Screw.rotation.x = Math.PI / 2;
  m2Screw.position.set(0, 0.004, m2Length - 0.006);
  m2Screw.visible = false;
  const m2Group = new THREE.Group();
  m2Group.position.copy(m2SlotPos);
  // Tilted up at the far (screw) end, like a drive that's plugged in but
  // not yet pressed flush and screwed down.
  m2Group.rotation.x = 0.3;
  m2Group.add(m2StickMesh);
  m2Group.add(m2Screw);
  m2Group.userData = { interactable: true, kind: "connector", id: "m2Seated" };
  internals.add(m2Group);
  ...
  // -------- Boot drive + its two unplugged SATA cables (Faults 2 & 3) --------
  // A simple 2.5" SATA SSD mounted in a drive bay, upper-left of the case
  // interior — well clear of the board (+X), PSU (further -X, lower) and
  // the M.2 fault above (near the board's own footprint).
  function buildBootDrive() {
    const group = new THREE.Group();
    const bodyTex = canvasTex(128, 96, (ctx, w, h) => {
      ctx.fillStyle = "#26292d";
      ctx.fillRect(0, 0, w, h);
      for (let i = 0; i < 60; i++) {
    ...
    const body = new THREE.Mesh(
      new THREE.BoxGeometry(0.014, 0.07, 0.1),
      new THREE.MeshStandardMaterial({ map: bodyTex, metalness: 0.5, roughness: 0.55 })
    );
    group.add(body);
    const labelTex = canvasTex(128, 96, (ctx, w, h) => {
      ctx.fillStyle = "#f4f2ea";
      ctx.fillRect(0, 0, w, h);
      ctx.fillStyle = "#111318";
      ctx.textAlign = "center";
      ctx.font = "bold 15px Arial";
      ctx.fillText("2.5\" SATA SSD", w / 2, h * 0.4);
      ctx.font = "11px Arial";
      ctx.fillStyle = "#33404f";
      ctx.fillText("BOOT DRIVE — Windows", w / 2, h * 0.62);
      ctx.fillText("480GB", w / 2, h * 0.78);
    });
    const label = new THREE.Mesh(
    ...
    label.rotation.y = Math.PI / 2;
    label.position.set(0.0075, 0, 0.005);
    group.add(label);

    const portMat = new THREE.MeshStandardMaterial({ color: 0x111214, roughness: 0.6 });
    const dataPort = new THREE.Mesh(new THREE.BoxGeometry(0.016, 0.012, 0.004), portMat);
    dataPort.position.set(0, -0.02, -0.048);
    group.add(dataPort);
    const powerPort = new THREE.Mesh(new THREE.BoxGeometry(0.016, 0.014, 0.004), portMat);
    powerPort.position.set(0, 0.008, -0.048);
    group.add(powerPort);
    group.userData.dataPortLocal = dataPort.position.clone();
    group.userData.powerPortLocal = powerPort.position.clone();
    return group;
  }
  const driveBayPos = new THREE.Vector3(
    chassisGroup.position.x - 0.12,
    chassisGroup.position.y + CH / 2 - 0.1,
    chassisGroup.position.z + 0.04
  );
  const bootDrive = buildBootDrive();
  bootDrive.position.copy(driveBayPos);
  internals.add(bootDrive);
  // Simple mounting rails, top and bottom of the bay, purely decorative.
  const railMat = new THREE.MeshStandardMaterial({ color: 0x2c2f33, metalness: 0.5, roughness: 0.6 });
  [0.037, -0.037].forEach((dy) => {
    const rail = new THREE.Mesh(new THREE.BoxGeometry(0.02, 0.006, 0.11), railMat);
    rail.position.set(driveBayPos.x, driveBayPos.y + dy, driveBayPos.z);
    internals.add(rail);
  });

  const dataPortWorld = driveBayPos.clone().add(bootDrive.userData.dataPortLocal);
  const powerPortWorld = driveBayPos.clone().add(bootDrive.userData.powerPortLocal);
  const sataHeaderPos = boardLocalToInternals(new THREE.Vector3(0.12, 0.012, 0.09));

  // A dynamically-updatable cable (its TubeGeometry is rebuilt each animation
  // frame) so the run can visibly follow its plug end from a dangling,
  // unplugged pose into the seated port instead of snapping or leaving a gap.
  function makeDynamicCable(anchor, endInit, radius, mat) {
    const mesh = new THREE.Mesh(new THREE.BufferGeometry(), mat);
    function update(end) {
      const mid = new THREE.Vector3().addVectors(anchor, end).multiplyScalar(0.5);
      mid.y -= 0.02;
      const curve = new THREE.CatmullRomCurve3([anchor, mid, end]);
      mesh.geometry.dispose();
      mesh.geometry = new THREE.TubeGeometry(curve, 14, radius, 6, false);
    }
    update(endInit);
    internals.add(mesh);
    return { update };
  ...
  // Flat ribbon-style SATA data cable — thin flattened box plug — dangling
  // below and out from its port.
  const dataDanglePos = dataPortWorld.clone().add(new THREE.Vector3(0, -0.035, 0.035));
  const sataDataCable = makeDynamicCable(sataHeaderPos, dataDanglePos, 0.0035, cableMat);
  const sataDataPlug = new THREE.Mesh(
    new THREE.BoxGeometry(0.016, 0.006, 0.012),
    new THREE.MeshStandardMaterial({ map: connTex, color: COLORS.connectorOff, roughness: 0.5 })
  );
  sataDataPlug.position.copy(dataDanglePos);
  sataDataPlug.userData = { interactable: true, kind: "connector", id: "sataData" };
  internals.add(sataDataPlug);

  // Round sleeved SATA power cable, run from the PSU — dangling lower still,
  // so the two unplugged cables read as visually distinct.
  const powerDanglePos = powerPortWorld.clone().add(new THREE.Vector3(0, -0.05, 0.035));
  const sataPowerCable = makeDynamicCable(psuTop, powerDanglePos, 0.004, cableMat);
  const sataPowerPlug = new THREE.Mesh(
    new THREE.BoxGeometry(0.016, 0.016, 0.012),
    new THREE.MeshStandardMaterial({ map: connTex, color: COLORS.connectorOff, roughness: 0.5 })
  );
  sataPowerPlug.position.copy(powerDanglePos);
  sataPowerPlug.userData = { interactable: true, kind: "connector", id: "sataPower" };
  internals.add(sataPowerPlug);
  ...
  function renderTaskList() {
    ...
    taskListEl.innerHTML = `<div class="tl-title">Storage Upgrade Repair</div><ul>${items}</ul>`;
  }
  ...
  function updateHint() {
    ...
    if (kind === "plug" && !taskState.unplug.done) text = `<kbd>E</kbd> Unplug from wall outlet`;
      ...
      if (id === "m2Seated" && !taskState.m2Seated.done) text = `<kbd>Click</kbd> Press the M.2 drive flush and screw it down`;
      if (id === "sataData" && !taskState.sataData.done) text = `<kbd>Click</kbd> Plug in the SATA data cable`;
      if (id === "sataPower" && !taskState.sataPower.done) text = `<kbd>Click</kbd> Plug in the SATA power cable`;
    }
  ...
  function repairsReady() {
    return taskState.m2Seated.done && taskState.sataData.done && taskState.sataPower.done;
  }
  ...
  function fixConnector(id) {
    if (id === "m2Seated") {
      let t = 0;
      const startRotX = m2Group.rotation.x;
      const anim = () => {
        t += 0.07;
        const tt = Math.min(1, t);
        m2Group.rotation.x = startRotX * (1 - tt);
        if (tt < 1) requestAnimationFrame(anim);
        else {
          m2Group.rotation.x = 0;
          m2Screw.visible = true;
          m2Group.userData.interactable = false;
          markDone("m2Seated");
          toast("M.2 drive pressed flush and screwed down");
        }
      };
      anim();
    } else if (id === "sataData") {
      let t = 0;
      const start = sataDataPlug.position.clone();
      const anim = () => {
        t += 0.09;
        const tt = Math.min(1, t);
        const cur = new THREE.Vector3().lerpVectors(start, dataPortWorld, tt);
        sataDataPlug.position.copy(cur);
        sataDataCable.update(cur);
        if (tt < 1) requestAnimationFrame(anim);
        else {
          sataDataPlug.material.color.setHex(COLORS.connectorOn);
          sataDataPlug.userData.interactable = false;
          markDone("sataData");
          toast("SATA data cable reconnected");
        }
      };
      anim();
    } else if (id === "sataPower") {
      let t = 0;
      const start = sataPowerPlug.position.clone();
      const anim = () => {
        t += 0.09;
        const tt = Math.min(1, t);
        const cur = new THREE.Vector3().lerpVectors(start, powerPortWorld, tt);
        sataPowerPlug.position.copy(cur);
        sataPowerCable.update(cur);
        if (tt < 1) requestAnimationFrame(anim);
        else {
          sataPowerPlug.material.color.setHex(COLORS.connectorOn);
          sataPowerPlug.userData.interactable = false;
          markDone("sataPower");
          toast("SATA power cable reconnected");
        }
      };
      anim();
    }
  ...
  function openDiagnoseModal() {
    ...
    wrap.innerHTML =
      ...
      `<p class="mc-body">Symptom: POSTs normally, but stops before Windows loads, since a second M.2 drive was installed. Select every issue you can see.</p>`;
