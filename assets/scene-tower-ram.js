...
const RAM_SPEC_SHEET = {
  ...
  rows: [
    ...
    ["Dual-channel (2 DIMM) install", "Use Slot A2 + Slot B2"],
    ["Quad-population (4 DIMM) install", "Use all 4 slots — but the two channels (A1/A2 and B1/B2) must each hold one stick of the matched kit; both sticks of a pair may not share one channel"],
...
const DIAGNOSE_OPTIONS = [
  {
    ...
    label: "One of the new RAM sticks isn't fully latched in its slot",
    explain: "Correct — an unlatched stick can lose contact under the slightest vibration, causing an inconsistent or failed boot.",
  },
  {
    id: "wrongPairing", correct: true,
    label: "RAM sticks are paired in the wrong slots for dual-channel",
    explain: "Correct — this board needs matched pairs split across A1/A2 and B1/B2 together; putting both new sticks in the same channel breaks the population the board expects.",
  },
  {
    id: "bentPin", correct: true,
    label: "A DIMM slot has a bent pin from a stick forced in at an angle",
    explain: "Correct — a bent contact pin can't reliably read that stick, which can single-handedly stop POST or cause a boot loop.",
  },
  {
    id: "psu", correct: false,
    label: "Power supply is failing",
    explain: "Not the cause here — the timing (immediately after a RAM upgrade) and boot-loop pattern point squarely at memory, not power delivery.",
  },
  {
    id: "cmos", correct: false,
    label: "CMOS battery is dead",
    explain: "Not the cause here — a dead CMOS battery causes lost BIOS settings over time, not a boot loop that started right after adding RAM.",
  },
];
...
const TASKS = [
  ...
  { id: "wrongPairing", label: "Move the RAM sticks into the correct slot pairing for dual-channel." },
...
export async function buildFPScene(container, { hintEl, toastEl, crosshairEl, taskListEl, wornItemsEl, modalOverlayEl, modalCardEl }) {
  ...
  const ticketTex = canvasTex(280, 360, (ctx, w, h) => {
    ...
    ctx.font = "bold 15px Arial";
    ctx.textAlign = "left";
    ctx.fillText("WORK ORDER  #2026-0096", 16, 28);
    ...
    ctx.fillText("RAM upgraded from 2 to 4 sticks by customer", 16, 82);
    ...
    const lines = [
      ...
      "since a 2-to-4-stick RAM upgrade.",
    ];
    ...
    const checks = [
      "New stick latched", "Slot pairing (A1/A2 vs B1/B2)", "Bent DIMM pins",
      "PSU health", "CMOS battery",
    ];
    checks.forEach((c, i) => {
      const y = 206 + i * 20;
  ...
  // Four DIMM slots (A1, A2, B1, B2 in board order) — this scenario's whole
  // fault story lives here. Two decoy sticks (A1, B2) are fine and NOT
  // interactable at all, so the hover fallback has real look-alike geometry
  // to correctly ignore; the other two slots each carry one of the three
  // faults (a bent pin shares B1 with the unlatched stick — the pin is what
  // kept that stick from seating flush in the first place).
  const dimmSlotsLocal = boardGroup.userData.dimmSlots;
  const slotPos = dimmSlotsLocal.map((v) => boardLocalToInternals(v));
  const slotA1Pos = slotPos[0], slotA2Pos = slotPos[1], slotB1Pos = slotPos[2], slotB2Pos = slotPos[3];

  // A1 — decoy, correctly seated, never touched. Purely a visual look-alike
  // for the hover-fallback test; deliberately NOT added to userData.interactable.
  const ramDecoyA1 = buildKingstonFuryRam();
  ramDecoyA1.position.copy(slotA1Pos);
  internals.add(ramDecoyA1);

  // A2 — one of the two new sticks, both wrongly crammed into channel A
  // (A1+A2) instead of being spread one-per-channel. Latched fine, just in
  // the wrong slot pairing. Fix: swap places with the B2 decoy below.
  const ramWrongPairing = buildKingstonFuryRam();
  ramWrongPairing.position.copy(slotA2Pos);
  ramWrongPairing.userData = { interactable: true, kind: "ram-fault", id: "wrongPairing" };
  internals.add(ramWrongPairing);

  // B1 — an old stick that was never fully latched (tilted, one clip open)
  // because a bent contact pin in this same slot kept blocking full travel.
  const RAM_TILT = THREE.MathUtils.degToRad(10);
  const RAM_POP = 0.014;
  const ramUnlatched = buildKingstonFuryRam();
  ramUnlatched.position.copy(slotB1Pos);
  ramUnlatched.position.y += RAM_POP;
  ramUnlatched.rotation.z = RAM_TILT;
  ramUnlatched.userData = { interactable: true, kind: "ram-fault", id: "notLatched" };
  internals.add(ramUnlatched);

  // Bent contact pin, same slot (B1) — a thin cylinder kicked out of the
  // slot channel at an angle instead of sitting flush, small enough that
  // it exercises the same hover-fallback path as the RAM sticks themselves.
  const bentPinMat = new THREE.MeshStandardMaterial({ color: 0xd4af37, metalness: 0.85, roughness: 0.25 });
  const bentPin = new THREE.Mesh(new THREE.CylinderGeometry(0.0015, 0.0015, 0.014, 8), bentPinMat);
  bentPin.position.copy(slotB1Pos);
  bentPin.position.x -= 0.004;
  bentPin.position.y -= 0.006;
  const BENT_PIN_ANGLE = THREE.MathUtils.degToRad(38);
  bentPin.rotation.x = BENT_PIN_ANGLE;
  bentPin.userData = { interactable: true, kind: "ram-fault", id: "bentPin" };
  internals.add(bentPin);

  // B2 — decoy, correctly seated old stick. Fix for wrongPairing swaps this
  // one into A2 while the A2 stick above moves here, so each channel ends
  // up with one old + one new stick.
  const ramDecoyB2 = buildKingstonFuryRam();
  ramDecoyB2.position.copy(slotB2Pos);
  internals.add(ramDecoyB2);
  ...
  // 8-pin CPU power connector — fully seated already in this scenario (not
  // one of this variant's faults), kept purely for visual completeness of
  // the cable run from the PSU.
  ...
  function renderTaskList() {
    ...
    taskListEl.innerHTML = `<div class="tl-title">Tower Workstation — RAM Upgrade</div><ul>${items}</ul>`;
  }
  ...
  function updateHint() {
    ...
    if (kind === "plug" && !taskState.unplug.done) text = `<kbd>E</kbd> Unplug from wall outlet`;
      ...
      if (id === "wrongPairing") text = `<kbd>Click</kbd> Swap into the correct channel pairing`;
      else if (id === "bentPin") text = `<kbd>Click</kbd> Straighten the bent pin`;
      else if (id === "notLatched") {
        text = taskState.bentPin.done
          ? `<kbd>Click</kbd> Push and latch the RAM stick`
          : `Straighten the bent pin in this slot first`;
      }
    }
  ...
  function repairsReady() {
    return taskState.notLatched.done && taskState.wrongPairing.done && taskState.bentPin.done;
  }
  ...
  // Swap the misplaced A2 stick with the correctly-seated B2 decoy so each
  // channel (A1/A2 and B1/B2) ends up with one old + one new stick instead
  // of both new sticks sharing channel A.
  function fixWrongPairing() {
    let t = 0;
    const startWrong = ramWrongPairing.position.clone();
    const startCorrect = ramDecoyB2.position.clone();
    const anim = () => {
      t += 0.025;
      const tt = Math.min(1, t);
      const e = tt < 0.5 ? 2 * tt * tt : 1 - Math.pow(-2 * tt + 2, 2) / 2;
      const lift = Math.sin(tt * Math.PI) * 0.02;
      ramWrongPairing.position.lerpVectors(startWrong, startCorrect, e);
      ramWrongPairing.position.y += lift;
      ramDecoyB2.position.lerpVectors(startCorrect, startWrong, e);
      ramDecoyB2.position.y += lift;
      if (tt < 1) requestAnimationFrame(anim);
      else {
        ramWrongPairing.position.copy(startCorrect);
        ramDecoyB2.position.copy(startWrong);
        ramWrongPairing.userData.interactable = false;
        markDone("wrongPairing");
        toast("Sticks swapped — each channel now has one old and one new stick");
      }
    };
    anim();
  }
  ...
  function fixBentPin() {
    let t = 0;
    const startRot = bentPin.rotation.x;
    const anim = () => {
      t += 0.08;
      const tt = Math.min(1, t);
      bentPin.rotation.x = startRot * (1 - tt);
      if (tt < 1) requestAnimationFrame(anim);
      else {
        bentPin.rotation.x = 0;
  ...
  function fixNotLatched() {
    if (!taskState.bentPin.done) {
      toast("Straighten the bent pin in that slot first");
      return;
    }
    let t = 0;
    const startY = ramUnlatched.position.y;
    const startRot = ramUnlatched.rotation.z;
    const anim = () => {
      t += 0.1;
      const tt = Math.min(1, t);
      ramUnlatched.position.y = startY - RAM_POP * tt;
      ramUnlatched.rotation.z = startRot * (1 - tt);
      if (tt < 1) requestAnimationFrame(anim);
      else {
        ramUnlatched.position.copy(slotB1Pos);
        ramUnlatched.rotation.z = 0;
        ramUnlatched.userData.interactable = false;
        markDone("notLatched");
  ...
  function openDiagnoseModal() {
    ...
    wrap.innerHTML =
      ...
      `<p class="mc-body">Symptom: powers on (fans + LEDs active), no display output, no POST beep, never reaches the OS. Select every issue you can see.</p>`;
  ...
  function tryPrimaryAction() {
    ...
    if (kind === "screw" && !removedScrews.has(id)) {
      ...
      if (id === "wrongPairing") fixWrongPairing();
