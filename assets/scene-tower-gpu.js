...
const GPU_SPEC_SHEET = {
  title: "ASUS TUF Gaming Z790-Plus WiFi — Discrete GPU Installation",
  rows: [
    ["PCIe slot", "Primary x16 slot, nearest the CPU"],
    ["Onboard video", "Automatically disabled once a discrete GPU is detected in the primary slot"],
    ["GPU power", "This card requires 1x 8-pin PCIe power connector directly from the PSU"],
    ["Seating", "Card must click into the slot's retention latch; unseated cards will not initialize."],
  ],
};

const DIAGNOSE_OPTIONS = [
  {
    id: "monitorPort", correct: true,
    label: "Monitor cable is still plugged into the motherboard's onboard video port, not the GPU",
    explain: "Correct — once a discrete GPU is installed, onboard video is disabled on most boards; the cable has to move to the GPU's output.",
  },
  {
    id: "gpuPower", correct: true,
    label: "GPU PCIe power connector is unplugged",
    explain: "Correct — a GPU that needs supplemental power won't initialize without it, even though the board itself still boots fine.",
  },
  {
    id: "gpuSeated", correct: true,
    label: "GPU is not fully seated in the PCIe x16 slot",
    explain: "Correct — a GPU sitting proud of the slot won't get a reliable PCIe link, so it can't output video even if it's powered.",
  },
  {
    id: "ram", correct: false,
    label: "RAM stick is bad or improperly seated",
    explain: "Not the cause here — the clean POST beep and normal boot behavior show memory is initializing fine.",
  },
  {
    id: "cmos", correct: false,
    label: "CMOS battery is dead",
    explain: "Not the cause here — a dead CMOS battery wouldn't produce a clean, consistent POST beep.",
  },
];
...
const TASKS = [
  ...
  { id: "monitorPort", label: "Move the monitor cable from the motherboard port to the GPU's port." },
  { id: "gpuPower", label: "Plug in the GPU's PCIe power connector." },
  { id: "gpuSeated", label: "Push the GPU fully into its slot and latch it." },
...
function buildTufMotherboard() {
  ...
  // Primary PCIe x16 slot, nearest the CPU, running along the board's lower
  // edge — where the discrete GPU plugs in for this scenario.
  const pcieSlotMat = new THREE.MeshStandardMaterial({ color: 0x18191c, roughness: 0.55 });
  const pcieSlot = new THREE.Mesh(new THREE.BoxGeometry(0.09, 0.01, 0.016), pcieSlotMat);
  pcieSlot.position.set(0.09, 0.008, 0.02);
  group.add(pcieSlot);
  const pcieLatchMat = new THREE.MeshStandardMaterial({ color: 0xc9cdd4, metalness: 0.6, roughness: 0.4 });
  const pcieLatch = new THREE.Mesh(new THREE.BoxGeometry(0.01, 0.014, 0.02), pcieLatchMat);
  pcieLatch.position.set(0.09 - 0.045 - 0.006, 0.013, 0.02);
  group.add(pcieLatch);
  group.userData.pcieSlotPos = new THREE.Vector3(0.09, 0.013, 0.02);
...
export async function buildFPScene(container, { hintEl, toastEl, crosshairEl, taskListEl, wornItemsEl, modalOverlayEl, modalCardEl }) {
  ...
  const ticketTex = canvasTex(280, 360, (ctx, w, h) => {
    ...
    ctx.font = "bold 15px Arial";
    ctx.textAlign = "left";
    ctx.fillText("WORK ORDER  #2026-0092", 16, 28);
    ...
    ctx.fillText("New GPU installed by customer", 16, 82);

    ctx.font = "bold 10px Arial";
    ctx.fillText("REPORTED SYMPTOM", 16, 104);
    ctx.font = "10px Arial";
    const lines = [
      "Powers on — fans spin, case LEDs",
      "lit, POST beep sounds — but monitor",
      "shows no signal since installing",
      "a new graphics card.",
    ];
    lines.forEach((line, i) => ctx.fillText(line, 16, 120 + i * 15));

    ctx.font = "bold 10px Arial";
    ctx.fillText("CHECK EVERY CAUSE YOU FIND:", 16, 190);
    const checks = [
      "Monitor cable port", "GPU PCIe power connector", "GPU seated in slot",
      "RAM seated", "CMOS battery",
    ];
    checks.forEach((c, i) => {
      const y = 206 + i * 20;
  ...
  // internals'/benchGroup's local space, so RAM/PCIe slot hotspots line up
  ...
  const ramSlotPos = boardLocalToInternals(dimmSlotsLocal[0]);
  const pciePos = boardLocalToInternals(boardGroup.userData.pcieSlotPos);

  // RAM is fine in this scenario (a diagnose distractor, not a fault) — it
  // sits correctly seated and is purely decorative, so it deliberately has
  // no interactable flag: it must never compete for hover/click priority
  // with the real fault objects.
  const ramStick = buildKingstonFuryRam();
  ramStick.position.copy(ramSlotPos);
  ...
  // CPU power is already fully seated in this scenario — decorative only,
  // same reasoning as the RAM stick above.
  ...
  // -------- Discrete GPU: this scenario's fault story --------
  // Card lives in the primary PCIe x16 slot, nearest the CPU. It starts
  // proud of the slot and cocked at a slight angle — visibly not seated —
  // the same visual language the base scenario uses for an unseated RAM
  // stick, so it reads immediately as "not fully plugged in".
  const GPU_THICKNESS = 0.028, GPU_HEIGHT = 0.05, GPU_LENGTH = 0.16;
  function buildGpuGroup() {
    const group = new THREE.Group();
    const shroudTex = canvasTex(256, 128, (ctx, w, h) => {
      ...
      for (let i = 0; i < 10; i++) {
        ctx.beginPath();
        ctx.moveTo((i / 10) * w, 0);
        ctx.lineTo((i / 10) * w - 16, h);
        ctx.stroke();
      }
      [0.28, 0.72].forEach((fx) => {
        const cx = w * fx, cy = h * 0.52, r = h * 0.4;
        ...
        for (let i = 0; i < 7; i++) {
          ...
          ctx.rotate(a);
          ctx.beginPath();
          ctx.ellipse(r * 0.55, 0, r * 0.45, r * 0.18, 0, 0, Math.PI * 2);
          ctx.fill();
          ctx.restore();
        }
        ctx.fillStyle = "#1c1e21";
      ...
      ctx.fillText("GPU", w / 2, h * 0.94);
    });
    const shroudMat = new THREE.MeshStandardMaterial({ map: shroudTex, color: 0xffffff, roughness: 0.6 });
    const sideMat = new THREE.MeshStandardMaterial({ color: COLORS.gpuShroud, roughness: 0.65 });
    // Box face order is [+x, -x, +y, -y, +z, -z]; +x is the face pointing
    // away from the board toward the open side panel, i.e. what the player
    // actually sees when looking into the case — that's the only face that
    // needs the fan/wordmark texture.
    const body = new THREE.Mesh(
      new THREE.BoxGeometry(GPU_THICKNESS, GPU_HEIGHT, GPU_LENGTH),
      [shroudMat, sideMat, sideMat, sideMat, sideMat, sideMat]
    );
    group.add(body);

    // Metal rear I/O bracket, occupying the case's rear expansion slot —
    // carries the GPU's own video-out port.
    const bracket = new THREE.Mesh(
      new THREE.BoxGeometry(GPU_THICKNESS * 0.5, GPU_HEIGHT * 0.92, 0.014),
      new THREE.MeshStandardMaterial({ color: COLORS.gpuBracket, metalness: 0.7, roughness: 0.35 })
    );
    bracket.position.set(GPU_THICKNESS * 0.1, 0, -GPU_LENGTH / 2 - 0.004);
    group.add(bracket);

    // Gold edge connector, where the card meets the slot.
    const contact = new THREE.Mesh(
      new THREE.BoxGeometry(GPU_THICKNESS * 0.85, 0.006, GPU_LENGTH * 0.55),
      new THREE.MeshStandardMaterial({ color: 0xd4af37, metalness: 0.85, roughness: 0.3 })
    );
    contact.position.set(0, -GPU_HEIGHT / 2 - 0.003, 0.006);
    group.add(contact);

    return group;
  }
  const gpuGroup = buildGpuGroup();
  const gpuSeatedPos = new THREE.Vector3(
    pciePos.x + GPU_THICKNESS / 2 + 0.004,
    pciePos.y + GPU_HEIGHT / 2,
    pciePos.z
  );
  const gpuUnseatedPos = gpuSeatedPos.clone().add(new THREE.Vector3(0.028, 0.012, 0));
  gpuGroup.position.copy(gpuUnseatedPos);
  gpuGroup.rotation.z = 0.14;
  gpuGroup.userData = { interactable: true, kind: "gpu-seat", id: "gpuSeated" };
  internals.add(gpuGroup);

  // GPU's supplemental PCIe power input, top-rear edge of the card — same
  // unplugged/plugged visual + interaction pattern as the CPU connector.
  const gpuPowerConn = new THREE.Mesh(
    new THREE.BoxGeometry(0.016, 0.022, 0.016),
    new THREE.MeshStandardMaterial({ map: connTex, color: COLORS.connectorOff, roughness: 0.5 })
  );
  gpuPowerConn.position.set(
    gpuSeatedPos.x + GPU_THICKNESS / 2 + 0.006,
    gpuSeatedPos.y + GPU_HEIGHT / 2 - 0.006,
    gpuSeatedPos.z - GPU_LENGTH / 2 + 0.03
  );
  gpuPowerConn.userData = { interactable: true, kind: "connector", id: "gpuPower" };
  internals.add(gpuPowerConn);

  // -------- Rear monitor ports: onboard video vs. the GPU's own output --------
  // Two distinguishable physical ports, well separated vertically so they
  // read as two different plugs, plus the cable prop that moves between them.
  const portMoboPos = new THREE.Vector3(chassisGroup.position.x + 0.05, chassisGroup.position.y + CH / 2 - 0.07, chassisGroup.position.z - CD / 2 + 0.002);
  const portGpuPos = new THREE.Vector3(gpuSeatedPos.x - GPU_THICKNESS * 0.3, gpuSeatedPos.y, chassisGroup.position.z - CD / 2 + 0.002);

  const moboPortMesh = new THREE.Mesh(
    new THREE.BoxGeometry(0.02, 0.014, 0.006),
    new THREE.MeshStandardMaterial({ color: COLORS.portMobo, roughness: 0.5 })
  );
  moboPortMesh.position.copy(portMoboPos);
  internals.add(moboPortMesh);

  const gpuPortMesh = new THREE.Mesh(
    new THREE.BoxGeometry(0.022, 0.014, 0.006),
    new THREE.MeshStandardMaterial({ color: COLORS.portGpu, roughness: 0.5 })
  );
  gpuPortMesh.position.copy(portGpuPos);
  internals.add(gpuPortMesh);

  const monitorPlugMat = new THREE.MeshStandardMaterial({ color: 0x2b2f34, roughness: 0.55 });
  const monitorCableMat = new THREE.MeshStandardMaterial({ color: 0x1c1e21, roughness: 0.75 });
  const monitorCableGroup = new THREE.Group();
  const monitorPlugBody = new THREE.Mesh(new THREE.BoxGeometry(0.017, 0.017, 0.022), monitorPlugMat);
  monitorPlugBody.position.z = -0.014;
  monitorCableGroup.add(monitorPlugBody);
  const monitorCableCurve = new THREE.CatmullRomCurve3([
    new THREE.Vector3(0, 0, -0.024),
    new THREE.Vector3(0, 0.02, -0.05),
    new THREE.Vector3(0.01, 0.05, -0.06),
  ]);
  monitorCableGroup.add(new THREE.Mesh(new THREE.TubeGeometry(monitorCableCurve, 12, 0.004, 6, false), monitorCableMat));
  monitorCableGroup.position.copy(portMoboPos).add(new THREE.Vector3(0, 0, -0.004));
  monitorCableGroup.userData = { interactable: true, kind: "monitor-cable", id: "monitorPort" };
  internals.add(monitorCableGroup);

  // Generic modular ATX PSU, mounted low and clear of the board's own
  // footprint (further -X, past where the board sits), with sleeved cables
  // run to the 24-pin edge, the 8-pin CPU connector, and the GPU's power input.
  ...
  // Supplemental GPU power run, from the PSU straight to the card's own
  // input — this is the run that's sitting unplugged in this scenario.
  addCable([
    psuTop,
    new THREE.Vector3(psuPos.x - 0.03, gpuPowerConn.position.y - 0.07, chassisGroup.position.z - CD / 2 + 0.02),
    new THREE.Vector3(gpuPowerConn.position.x - 0.012, gpuPowerConn.position.y - 0.01, gpuPowerConn.position.z),
  ], 0.009);
  ...
  function renderTaskList() {
    ...
    taskListEl.innerHTML = `<div class="tl-title">Tower Workstation — GPU Upgrade</div><ul>${items}</ul>`;
  }
  ...
  function updateHint() {
    ...
    else if (kind === "gpu-seat" && taskState.diagnose.done && !taskState.gpuSeated.done) {
      text = `<kbd>Click</kbd> Push GPU fully into the slot and latch it`;
    } else if (kind === "monitor-cable" && taskState.diagnose.done && !taskState.monitorPort.done) {
      text = `<kbd>Click</kbd> Move monitor cable to the GPU's video port`;
    } else if (kind === "connector" && taskState.diagnose.done) {
      if (id === "gpuPower" && !taskState.gpuPower.done) text = `<kbd>Click</kbd> Plug in GPU power connector`;
    }
  ...
  function repairsReady() {
    return taskState.monitorPort.done && taskState.gpuPower.done && taskState.gpuSeated.done;
  }
  ...
  function seatGpu() {
    let t = 0;
    const startPos = gpuGroup.position.clone();
    const startRot = gpuGroup.rotation.z;
    const anim = () => {
      t += 0.06;
      const tt = Math.min(1, t);
      const e = tt < 0.5 ? 2 * tt * tt : 1 - Math.pow(-2 * tt + 2, 2) / 2;
      gpuGroup.position.lerpVectors(startPos, gpuSeatedPos, e);
      gpuGroup.rotation.z = startRot * (1 - e);
      if (tt < 1) requestAnimationFrame(anim);
      else {
        gpuGroup.position.copy(gpuSeatedPos);
        gpuGroup.rotation.z = 0;
        markDone("gpuSeated");
        toast("GPU pushed fully into the slot and latched");
      }
    };
    anim();
  }

  function moveMonitorCable() {
    let t = 0;
    const start = monitorCableGroup.position.clone();
    const target = portGpuPos.clone().add(new THREE.Vector3(0, 0, -0.004));
    const anim = () => {
      t += 0.05;
      const tt = Math.min(1, t);
      const e = tt < 0.5 ? 2 * tt * tt : 1 - Math.pow(-2 * tt + 2, 2) / 2;
      monitorCableGroup.position.lerpVectors(start, target, e);
      if (tt < 1) requestAnimationFrame(anim);
      else {
        monitorCableGroup.position.copy(target);
        markDone("monitorPort");
        toast("Monitor cable moved to the GPU's video output");
      }
    };
    anim();
  }
  ...
  function fixConnector(id) {
    if (id === "gpuPower") {
      gpuPowerConn.material.color.setHex(COLORS.connectorOn);
      gpuPowerConn.position.x -= 0.005;
      markDone("gpuPower");
      toast("GPU PCIe power connector plugged in");
    }
  }
  ...
  function openManualModal() {
    const wrap = el("div");
    wrap.innerHTML = `<div class="mc-kicker">Reference</div><h2>${GPU_SPEC_SHEET.title}</h2>`;
    const table = el("table", "spec-table");
    GPU_SPEC_SHEET.rows.forEach(([label, value]) => {
      const tr = el("tr");
  ...
  function openDiagnoseModal() {
    ...
    wrap.innerHTML =
      ...
      `<p class="mc-body">Symptom: powers on — fans spin, case LEDs lit, POST beep sounds — but the monitor shows no signal since a new graphics card was installed. Select every issue you can see.</p>`;
