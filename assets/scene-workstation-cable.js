...
const CABLE_SPEC_SHEET = {
  title: "ASUS TUF Gaming Z790-Plus WiFi — Storage & Cable Maintenance",
  rows: [
    ["Model", "ASUS TUF Gaming Z790-Plus WiFi (Intel LGA1700, Z790)"],
    ["SATA data cable", "Inspect connector housings at both ends for cracks or splits — a cracked housing can still seat but make an intermittent electrical connection"],
    ["Drive vibration mount", "Rubber dampening mounts degrade with age and heat; a crumbled mount lets the drive vibrate more, which can work a marginal cable connection loose over time"],
    ["SATA power connector", "Inspect pins for oxidation (dulled, spotted, discolored) — oxidized pins raise contact resistance and can cause intermittent power loss to a drive"],
    ["Diagnosis tip", "Symptoms isolated to one drive — intermittent errors, occasional non-detection — with the rest of the system stable point to that drive's own connections, not board-wide power or memory"],
  ],
};

const DIAGNOSE_OPTIONS = [
  {
    id: "sataDataCracked", correct: true,
    label: "SATA data cable connector is cracked",
    explain: "Correct — a cracked connector housing can create an intermittent data connection, causing sporadic drive errors and occasional non-detection.",
  },
  {
    id: "vibrationMount", correct: true,
    label: "Drive vibration-dampening mount has degraded",
    explain: "Correct — a failed mount lets the drive vibrate more than intended, which can shake a marginal cable connection loose over time.",
  },
  {
    id: "sataPowerOxidized", correct: true,
    label: "SATA power connector pins are oxidized",
    explain: "Correct — oxidized pins increase resistance and can cause an intermittent power connection to the drive, especially under vibration.",
  },
  {
    id: "ram", correct: false,
    label: "RAM stick is bad or improperly seated",
    explain: "Not the cause here — the symptom is specifically drive detection/errors, not general instability suggesting memory.",
  },
  {
    id: "power24pin", correct: false,
    label: "24-pin main power connector is loose",
    explain: "Not the cause here — the system boots and runs fine otherwise; the issue is isolated to storage connections, not overall board power.",
  },
];
...
const TASKS = [
  ...
  { id: "sataDataCracked", label: "Replace the cracked SATA data cable connector." },
  { id: "vibrationMount", label: "Replace the failed drive vibration-dampening mount." },
  { id: "sataPowerOxidized", label: "Clean the oxidized SATA power connector pins." },
...
export async function buildFPScene(container, { hintEl, toastEl, crosshairEl, taskListEl, wornItemsEl, modalOverlayEl, modalCardEl }) {
  ...
  const ticketTex = canvasTex(280, 360, (ctx, w, h) => {
    ...
    ctx.fillText("Years of continuous service", 16, 82);

    ctx.font = "bold 10px Arial";
    ctx.fillText("REPORTED SYMPTOM", 16, 104);
    ctx.font = "10px Arial";
    const lines = [
      "Intermittent drive errors; sometimes",
      "the boot drive isn't detected at all.",
      "Rest of system boots/runs fine.",
      "Symptom has worsened over weeks.",
    ];
    ...
    const checks = [
      "SATA data cable condition", "Drive vibration mount", "SATA power connector pins",
      "RAM seating", "24-pin power connector",
    ];
    checks.forEach((c, i) => {
      const y = 216 + i * 20;
  ...
  // RAM stick — a distractor for this scenario's diagnosis, so it's shown
  // fully and correctly seated (no fault, no interaction) rather than the
  // partially-seated fault used in the base workstation scenario.
  ...
  // The 24-pin main power connector is a distractor for this scenario —
  // shown fully seated and connected, not part of the fault set.
  ...
  // -------- Storage drive assembly: the three cable-degradation faults --------
  // A 2.5" drive mounted in the case's front-upper bay, with its own SATA
  // data cable, SATA power cable, and rubber vibration-dampening mounts —
  // the three things that actually fail in this scenario.
  const driveBayPos = new THREE.Vector3(
    chassisGroup.position.x + 0.11,
    chassisGroup.position.y + 0.12,
    chassisGroup.position.z + 0.06
  );
  const driveLabelTex = canvasTex(160, 128, (ctx, w, h) => {
    ...
    ctx.fillText("2.5\" SATA SSD", w / 2, h * 0.42);
    ctx.font = "10px Arial";
    ctx.fillStyle = "#54585e";
    ctx.fillText("500GB — SATA III 6Gb/s", w / 2, h * 0.62);
  });
  const driveShellMat = new THREE.MeshStandardMaterial({ color: 0x26292d, roughness: 0.55 });
  const driveLabelMat = new THREE.MeshStandardMaterial({ map: driveLabelTex, roughness: 0.6 });
  const driveMesh = new THREE.Mesh(
    new THREE.BoxGeometry(0.09, 0.012, 0.065),
    [driveShellMat, driveShellMat, driveLabelMat, driveShellMat, driveShellMat, driveShellMat]
  );
  driveMesh.position.copy(driveBayPos);
  internals.add(driveMesh);

  // Points on the motherboard edge where the storage cables terminate.
  const sataPortPos = boardLocalToInternals(new THREE.Vector3(0.13, 0.012, -0.08));

  function buildSataConnector(tex, colorHex, rough) {
    return new THREE.Mesh(
      new THREE.BoxGeometry(0.016, 0.011, 0.02),
      new THREE.MeshStandardMaterial({ map: tex, color: colorHex, roughness: rough })
    );
  }

  // Fault 1: cracked SATA data cable connector, right at the drive's data
  // port. A visible jagged split across the housing, replaced with a clean
  // connector + fresh cable once fixed.
  const crackedConnTex = canvasTex(64, 64, (ctx, w, h) => {
    ctx.fillStyle = "#1c1e21";
    ctx.fillRect(0, 0, w, h);
    ctx.fillStyle = "#0a0b0c";
    for (let x = 6; x < w - 6; x += 8) for (let y = 6; y < h - 6; y += 8) ctx.fillRect(x, y, 3, 3);
    const crackPts = [[0.1, 0.15], [0.35, 0.4], [0.28, 0.55], [0.55, 0.75], [0.5, 0.95]];
    ctx.lineJoin = "round";
    ctx.strokeStyle = "rgba(0,0,0,0.65)";
    ctx.lineWidth = 3.4;
    ctx.beginPath();
    crackPts.forEach(([px, py], i) => (i === 0 ? ctx.moveTo(px * w, py * h) : ctx.lineTo(px * w, py * h)));
    ctx.stroke();
  ...
  const dataPortPos = new THREE.Vector3(driveBayPos.x - 0.05, driveBayPos.y - 0.002, driveBayPos.z - 0.02);
  addCable([
    dataPortPos,
    new THREE.Vector3((dataPortPos.x + sataPortPos.x) / 2, Math.max(dataPortPos.y, sataPortPos.y) + 0.03, (dataPortPos.z + sataPortPos.z) / 2),
    sataPortPos,
  ], 0.01, false);
  const crackedDataConn = buildSataConnector(crackedConnTex, 0x9098a0, 0.55);
  crackedDataConn.position.copy(dataPortPos);
  crackedDataConn.userData = { interactable: true, kind: "storage-fault", id: "sataDataCracked" };
  internals.add(crackedDataConn);
  const freshDataConn = buildSataConnector(connTex, 0xd8dadd, 0.35);
  freshDataConn.position.copy(dataPortPos);
  freshDataConn.visible = false;
  internals.add(freshDataConn);

  // Fault 2: degraded rubber vibration-dampening mounts underneath the
  // drive — crumbled/discolored until swapped for a fresh set.
  const crumbledRubberTex = canvasTex(64, 64, (ctx, w, h) => {
    ctx.fillStyle = "#4a4038";
    ctx.fillRect(0, 0, w, h);
    for (let i = 0; i < 160; i++) {
      ctx.fillStyle = `rgba(${80 + Math.random() * 40},${60 + Math.random() * 30},${40 + Math.random() * 20},${0.15 + Math.random() * 0.25})`;
      ctx.beginPath();
    ...
    for (let i = 0; i < 7; i++) {
      ...
      ctx.beginPath();
      ctx.moveTo(x, y);
      for (let s = 0; s < 3; s++) { x += (Math.random() - 0.5) * 14; y += (Math.random() - 0.5) * 14; ctx.lineTo(x, y); }
      ctx.stroke();
    }
  });
  const freshRubberTex = canvasTex(64, 64, (ctx, w, h) => {
    ctx.fillStyle = "#1c1e21";
    ctx.fillRect(0, 0, w, h);
    ctx.fillStyle = "rgba(255,255,255,0.05)";
    ctx.beginPath();
    ctx.arc(w * 0.35, h * 0.3, w * 0.12, 0, Math.PI * 2);
    ctx.fill();
  });
  function buildMountPads(tex, rough) {
    const g = new THREE.Group();
    [-0.03, 0.03].forEach((dx) => {
      const pad = new THREE.Mesh(
        new THREE.CylinderGeometry(0.009, 0.011, 0.009, 14),
        new THREE.MeshStandardMaterial({ map: tex, roughness: rough })
      );
      pad.position.set(dx, 0, 0);
      g.add(pad);
    });
    return g;
  }
  const mountPos = new THREE.Vector3(driveBayPos.x, driveBayPos.y - 0.009, driveBayPos.z);
  const crumbledMount = buildMountPads(crumbledRubberTex, 0.95);
  crumbledMount.position.copy(mountPos);
  crumbledMount.userData = { interactable: true, kind: "storage-fault", id: "vibrationMount" };
  internals.add(crumbledMount);
  const freshMount = buildMountPads(freshRubberTex, 0.6);
  freshMount.position.copy(mountPos);
  freshMount.visible = false;
  internals.add(freshMount);

  // Fault 3: oxidized SATA power connector pins — cleaned and reseated in
  // place (animated) rather than swapped for a new part.
  const oxidizedPinsTex = canvasTex(64, 64, (ctx, w, h) => {
    ctx.fillStyle = "#9aa0a8";
    ctx.fillRect(0, 0, w, h);
    ctx.fillStyle = "#6b7178";
    for (let x = 4; x < w - 4; x += 6) ctx.fillRect(x, 4, 2, h - 8);
    for (let i = 0; i < 40; i++) {
      ctx.fillStyle = `rgba(${90 + Math.random() * 40},${110 + Math.random() * 40},${60 + Math.random() * 30},${0.25 + Math.random() * 0.35})`;
      ctx.beginPath();
      ctx.arc(Math.random() * w, Math.random() * h, 1.5 + Math.random() * 3, 0, Math.PI * 2);
      ctx.fill();
    }
  });
  const cleanPinsTex = canvasTex(64, 64, (ctx, w, h) => {
    ctx.fillStyle = "#c9cdd4";
    ctx.fillRect(0, 0, w, h);
    ctx.fillStyle = "#e8e9eb";
    for (let x = 4; x < w - 4; x += 6) ctx.fillRect(x, 4, 2, h - 8);
  });
  const powerPortPos = new THREE.Vector3(driveBayPos.x - 0.05, driveBayPos.y - 0.002, driveBayPos.z + 0.02);
  addCable([
    psuTop,
    new THREE.Vector3(psuPos.x + 0.05, psuPos.y + 0.15, psuPos.z + 0.03),
    powerPortPos,
  ], 0.011, false);
  const sataPowerConn = new THREE.Mesh(
    new THREE.BoxGeometry(0.018, 0.012, 0.016),
    new THREE.MeshStandardMaterial({ map: oxidizedPinsTex, roughness: 0.6 })
  );
  sataPowerConn.position.copy(powerPortPos);
  sataPowerConn.userData = { interactable: true, kind: "storage-fault", id: "sataPowerOxidized" };
  internals.add(sataPowerConn);
  ...
  function renderTaskList() {
    ...
    taskListEl.innerHTML = `<div class="tl-title">Workstation — Drive Errors</div><ul>${items}</ul>`;
  }
  ...
  function updateHint() {
    ...
    else if (kind === "storage-fault" && !taskState[id]?.done) {
      if (!taskState.diagnose.done) text = `Diagnose the fault first`;
      else if (id === "sataDataCracked") text = `<kbd>Click</kbd> Replace cracked SATA data cable connector`;
      else if (id === "vibrationMount") text = `<kbd>Click</kbd> Replace degraded vibration-dampening mount`;
      else if (id === "sataPowerOxidized") text = `<kbd>Click</kbd> Clean and reseat oxidized SATA power connector`;
    }
  ...
  function repairsReady() {
    return taskState.sataDataCracked.done && taskState.vibrationMount.done && taskState.sataPowerOxidized.done;
  }
  ...
  function fixStorageFault(id) {
    if (id === "sataDataCracked") {
      crackedDataConn.visible = false;
      freshDataConn.visible = true;
      markDone("sataDataCracked");
      toast("SATA data cable connector replaced");
    } else if (id === "vibrationMount") {
      crumbledMount.visible = false;
      freshMount.visible = true;
      markDone("vibrationMount");
      toast("Vibration-dampening mount replaced");
    } else if (id === "sataPowerOxidized") {
      const startPos = sataPowerConn.position.clone();
      const outPos = startPos.clone().add(new THREE.Vector3(-0.025, 0, 0));
      let t = 0;
      const anim = () => {
        t += 0.08;
        const tt = Math.min(1, t);
        if (tt < 0.5) {
          sataPowerConn.position.lerpVectors(startPos, outPos, tt / 0.5);
        } else {
          if (sataPowerConn.material.map !== cleanPinsTex) {
            sataPowerConn.material.map = cleanPinsTex;
            sataPowerConn.material.needsUpdate = true;
          }
          sataPowerConn.position.lerpVectors(outPos, startPos, (tt - 0.5) / 0.5);
        }
        if (tt < 1) requestAnimationFrame(anim);
        else {
          sataPowerConn.position.copy(startPos);
          markDone("sataPowerOxidized");
          toast("SATA power connector pins cleaned and reseated");
        }
      };
      anim();
    }
  ...
  function openManualModal() {
    const wrap = el("div");
    wrap.innerHTML = `<div class="mc-kicker">Reference</div><h2>${CABLE_SPEC_SHEET.title}</h2>`;
    const table = el("table", "spec-table");
    CABLE_SPEC_SHEET.rows.forEach(([label, value]) => {
      const tr = el("tr");
  ...
  function openDiagnoseModal() {
    ...
    wrap.innerHTML =
      ...
      `<p class="mc-body">Symptom: intermittent drive errors, and sometimes the boot drive isn't detected at all. The rest of the system boots and runs fine. Symptom has worsened over recent weeks. Select every issue you can see.</p>`;
  ...
  function tryPrimaryAction() {
    ...
    if (kind === "screw" && !removedScrews.has(id)) {
      ...
      unscrew(hovered);
    } else if (kind === "storage-fault" && taskState.diagnose.done && !taskState[id]?.done) {
      fixStorageFault(id);
