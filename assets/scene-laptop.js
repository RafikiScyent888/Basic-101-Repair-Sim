...
const RAM_SPEC_SHEET = {
  ...
  rows: [
    ...
    ["Dual-channel (2 DIMM) install", "Use Slot 1 + Slot 2"],
    ["Supported speeds", "DDR5-4800 to DDR5-5600 native"],
  ],
};

const DIAGNOSE_OPTIONS = [
  {
    id: "ramSlot", correct: true,
    label: "RAM stick is installed in a slot the board doesn't read at boot for a single-stick config",
    explain: "Correct — check the manual: with only one stick installed, this board only reads Slot 1 at POST.",
  },
  {
    id: "displayCable", correct: true,
    label: "Internal display cable is unplugged from the motherboard",
    explain: "Correct — without the display cable connected, the system boots normally but never sends a signal to the panel.",
  },
  {
    id: "stray", correct: true,
    label: "A stray screw underneath the board is shorting it against the chassis",
    explain: "Correct — a loose screw not seated in a standoff hole shorts the board against the chassis.",
  },
  {
    id: "battery", correct: false,
    label: "Battery is fully dead",
    explain: "Not the cause here — a dead battery wouldn't stop the fan and power LED from working on AC power alone.",
  },
  {
    id: "cmos", correct: false,
    label: "CMOS battery is dead",
    explain: "Not the cause here — a dead CMOS battery on a laptop just resets BIOS/clock settings, not the entire display output.",
  },
];
...
const TASKS = [
  ...
  { id: "ram", label: "Move the RAM stick to the correct slot and seat it." },
  { id: "displayCable", label: "Reconnect the internal display cable." },
  { id: "stray", label: "Remove the stray screw shorting the board." },
...
export async function buildFPScene(container, { hintEl, toastEl, crosshairEl, taskListEl, wornItemsEl, modalOverlayEl, modalCardEl }) {
  ...
  const ticketTex = canvasTex(280, 360, (ctx, w, h) => {
    ...
    ctx.font = "bold 15px Arial";
    ctx.textAlign = "left";
    ctx.fillText("WORK ORDER  #2026-0417", 16, 28);
    ...
    ctx.font = "bold 10px Arial";
    ctx.fillText("REPORTED SYMPTOM", 16, 92);
    ctx.font = "10px Arial";
    const lines = [
      "Powers on — fan spins, power LED",
      "lit, keyboard backlight on — but",
      "screen stays black. No image,",
      "not even the manufacturer boot",
      "logo. Never reaches the OS.",
    ];
    lines.forEach((line, i) => ctx.fillText(line, 16, 108 + i * 15));

    ctx.font = "bold 10px Arial";
    ctx.fillText("CHECK EVERY CAUSE YOU FIND:", 16, 200);
    const checks = [
      "RAM slot", "Display cable", "Stray screw / short",
  ...
  const slot1Pos = slotPositions[0]; // correct — closest to CPU
  const slot2Pos = slotPositions[1];

  const slot1Hotspot = new THREE.Object3D();
  slot1Hotspot.position.copy(slot1Pos);
  slot1Hotspot.userData = { interactable: true, kind: "ram-slot", id: "slot1" };
  internals.add(slot1Hotspot);
  const slot2Hotspot = new THREE.Object3D();
  slot2Hotspot.position.copy(slot2Pos);
  slot2Hotspot.userData = { interactable: true, kind: "ram-slot", id: "slot2" };
  internals.add(slot2Hotspot);

  const ramStick = buildATechRam();
  ramStick.rotation.z = Math.PI / 2;
  ramStick.position.copy(slot2Pos);
  ramStick.userData = { interactable: true, kind: "ram", id: "ramStick" };
  ...
  displayConn.userData = { interactable: true, kind: "connector", id: "displayCable" };
  ...
  standoff.userData = { interactable: true, kind: "connector", id: "stray" };
  ...
  function renderTaskList() {
    ...
    const html = `<div class="tl-title">Laptop No-Boot Repair</div><ul>${TASKS.map((t) => {
  ...
  function updateHint() {
    ...
    else if (kind === "ram" && !ramInserted) {
      if (!taskState.diagnose.done) text = `Diagnose the fault first`;
      else if (carriedPartMesh) text = `Hands full`;
      else text = `<kbd>E</kbd> Pull RAM stick`;
    } else if (kind === "ram-slot") {
      if (carriedPartKind === "ram") text = id === "slot1" ? `<kbd>E</kbd> Insert into Slot 1` : `Wrong slot — check the manual`;
    } else if (kind === "ram-seat" && !taskState.ram.done) {
      text = `<kbd>Click</kbd> Push down evenly until it clicks into place`;
    } else if (kind === "connector" && taskState.diagnose.done) {
      if (id === "displayCable" && !taskState.displayCable.done) text = `<kbd>Click</kbd> Reconnect display cable`;
      if (id === "stray" && !taskState.stray.done) text = `<kbd>Click</kbd> Remove shorting screw`;
    }
  ...
  function repairsReady() {
    return taskState.ram.done && taskState.displayCable.done && taskState.stray.done;
  }
  ...
  function pullRam() {
    ramStick.visible = false;
    const carried = buildATechRam();
    carried.rotation.z = Math.PI / 2;
    partHandAnchor.add(carried);
    carriedPartMesh = carried;
    carriedPartKind = "ram";
    toast("Pulled the RAM stick");
  }

  function insertRam(slotId) {
    if (slotId !== "slot1") {
      toast("That's not the right slot — check the manual");
      return;
    }
    ...
    carriedPartKind = null;
    ramInserted = true;
    ramStick.visible = true;
    ramStick.position.copy(slot1Pos);
    ramStick.position.y += 0.03;
    ramStick.userData = { interactable: true, kind: "ram-seat", id: "ramSeatConfirm" };
    toast("RAM inserted into Slot 1 — push down to seat it");
  }

  function seatRam() {
    let t = 0;
    const startY = ramStick.position.y;
    const anim = () => {
      t += 0.15;
      ramStick.position.y = startY + (slot1Pos.y - startY) * Math.min(1, t);
      if (t < 1) requestAnimationFrame(anim);
      else { markDone("ram"); toast("RAM clicked into place"); }
    };
    anim();
  }
  ...
  function openDiagnoseModal() {
    ...
    wrap.innerHTML =
      ...
      `<p class="mc-body">Symptom: powers on (fan spins, power LED lit, keyboard backlight on), no display output, never reaches the OS. Select every issue you can see.</p>`;
  ...
  function trySecondaryAction() {
    ...
    if (kind === "grab-tool" && heldTool !== id) pickUpTool(hovered);
      ...
      openDiagnoseModal();
    } else if (kind === "ram" && !ramInserted && taskState.diagnose.done && !carriedPartMesh) {
      pullRam();
    } else if (kind === "ram-slot" && carriedPartKind === "ram") {
      insertRam(id);
    }
  }
