...
const STORAGE_SPEC_SHEET = {
  title: "Razer Blade 18 — Storage Configuration",
  rows: [
    ["Model", "Razer Blade 18 (2026), dual M.2 2280 NVMe storage"],
    ["M.2 slots", "2x M.2 2280 PCIe 5.0 x4, M-key — Slot 1 (boot) / Slot 2 (secondary)"],
    ["Boot drive", "Windows install lives on the original drive — do not relocate it when adding storage"],
    ["Mounting screw", "Each M.2 drive requires its own retaining screw; an unscrewed drive can flex out of contact with the connector"],
    ["Installing a second drive", "Handle the original drive's connector carefully while working nearby — it can be bumped loose during the upgrade"],
  ],
};

const DIAGNOSE_OPTIONS = [
  {
    id: "newSsdSeated", correct: true,
    label: "The new M.2 SSD isn't fully seated in its slot",
    explain: "Correct — the new drive is resting at an angle and isn't making contact with the connector, so the board can't see it.",
  },
  {
    id: "newSsdScrewed", correct: true,
    label: "The new M.2 SSD's retaining screw was never installed",
    explain: "Correct — without the mounting screw the drive can flex up out of its connector under vibration or handling.",
  },
  {
    id: "origSsdReseated", correct: true,
    label: "Installing the new drive bumped the original boot SSD's connector loose",
    explain: "Correct — the original Windows boot drive's connector got jostled loose during the upgrade and needs to be reseated.",
  },
  {
    id: "ram", correct: false,
    label: "RAM stick is bad or improperly seated",
    explain: "Not the cause here — a clean POST confirms memory is initializing fine.",
  },
  {
    id: "battery", correct: false,
    label: "Battery/charger is failing",
    explain: "Not the cause here — the system boots and POSTs consistently; this points to storage, not power.",
  },
];
...
const TASKS = [
  ...
  { id: "newSsdSeated", label: "Press the new M.2 SSD flush into its slot." },
  { id: "newSsdScrewed", label: "Install the missing retaining screw on the new M.2 SSD." },
  { id: "origSsdReseated", label: "Reseat the original boot SSD's connector, bumped loose during the upgrade." },
...
// A-Tech DDR5 SO-DIMM: the #5 pick from a 2026 laptop-RAM roundup. Real
// SO-DIMMs ship bare (no heatsink) — a dark PCB, small ICs on the face,
// and a plain printed label, unlike a desktop DIMM's finned cooler.
// Kept here purely as decoration (permanently seated, non-interactive) —
// this scenario's fault is storage, not memory.
function buildATechRam() {
  const group = new THREE.Group();
  const pcbW = 0.003, pcbH = 0.03, pcbD = 0.068;

  const faceTex = canvasTex(256, 128, (ctx, w, h) => {
    ctx.fillStyle = "#14161a";
    ctx.fillRect(0, 0, w, h);
    ctx.fillStyle = "#1c2024";
    for (let i = 0; i < 8; i++) {
      ctx.fillRect(w * 0.06 + i * w * 0.11, h * 0.15, w * 0.07, h * 0.4);
    }
    ctx.fillStyle = "#c9cdd4";
    ctx.textAlign = "center";
    ctx.font = "bold 13px Arial";
    ctx.fillText("A-TECH", w / 2, h * 0.72);
    ctx.font = "9px Arial";
    ctx.fillStyle = "#8a9098";
    ctx.fillText("DDR5 SO-DIMM  •  16GB  •  PC5-38400", w / 2, h * 0.86);
  });
  const pcb = new THREE.Mesh(
    new THREE.BoxGeometry(pcbW, pcbH, pcbD),
    new THREE.MeshStandardMaterial({ map: faceTex, roughness: 0.7, side: THREE.DoubleSide })
  );
  group.add(pcb);

  const contactTex = canvasTex(128, 16, (ctx, w, h) => {
    ...
    ctx.strokeStyle = "rgba(0,0,0,0.35)";
    ctx.lineWidth = 1;
    for (let x = 2; x < w; x += 3) { ctx.beginPath(); ctx.moveTo(x, 0); ctx.lineTo(x, h); ctx.stroke(); }
  });
  const contact = new THREE.Mesh(
    new THREE.BoxGeometry(pcbW * 1.4, 0.004, pcbD * 0.94),
    new THREE.MeshStandardMaterial({ map: contactTex, metalness: 0.7, roughness: 0.35 })
  );
  contact.position.y = -pcbH / 2 - 0.001;
  group.add(contact);

  group.userData.footprint = { w: pcbW, h: pcbH, d: pcbD };
  return group;
}

// M.2 NVMe stick — a thin labeled PCB stick, long axis along local Z so it
// can be parented at its connector end and pivoted about that end to show
// "resting at an angle, not yet seated".
function buildM2Stick(labelLines, lengthZ) {
  const tex = canvasTex(32, 256, (ctx, w, h) => {
    ctx.fillStyle = "#14161a";
    ctx.fillRect(0, 0, w, h);
    ctx.fillStyle = "#1c2024";
    for (let i = 0; i < 5; i++) ctx.fillRect(w * 0.15, h * (0.1 + i * 0.16), w * 0.7, h * 0.08);
    ctx.save();
    ...
    labelLines.forEach((line, i) => ctx.fillText(line, 0, (i - (labelLines.length - 1) / 2) * 11));
    ctx.restore();
  });
  const stick = new THREE.Mesh(
    new THREE.BoxGeometry(0.011, 0.0012, lengthZ),
...
export async function buildFPScene(container, { hintEl, toastEl, crosshairEl, taskListEl, wornItemsEl, modalOverlayEl, modalCardEl }) {
  ...
  const ticketTex = canvasTex(280, 360, (ctx, w, h) => {
    ...
    ctx.fillText("Second M.2 SSD installed by customer", 16, 84);

    ctx.font = "bold 10px Arial";
    ctx.fillText("REPORTED SYMPTOM", 16, 104);
    ctx.font = "10px Arial";
    const lines = [
      "Powers on and POSTs normally,",
      "but stops before Windows loads,",
      "since a second M.2 SSD was",
      "installed.",
    ];
    ...
    const checks = [
      "New SSD seated/screwed down", "Original SSD seated",
      "M.2 slot selection", "RAM seated", "Battery/charger health",
    ];
    checks.forEach((c, i) => {
      const y = 216 + i * 20;
  ...
  const panelVentTex = canvasTex(128, 96, (ctx, w, h) => {
    ...
    ctx.font = "6px Arial";
    ctx.fillText("MODEL RZ09-0484  S/N 2026-0512-A", w * 0.5, h * 0.42);
  ...
  // Two SO-DIMM slots — purely decorative in this scenario (the fault is
  // storage, not memory). Slot 1 carries a permanently-seated stick so a
  // player who suspects RAM can look and see it's already fine; Slot 2
  // stays visibly empty, same as a real single-stick config.
  ...
  const slotPositions = [
    new THREE.Vector3(-LW * 0.02, -0.0025, -LD * 0.4),
    new THREE.Vector3(-LW * 0.02, -0.0025, -LD * 0.12),
  ];
  slotPositions.forEach((p, i) => {
    const slot = new THREE.Mesh(new THREE.BoxGeometry(0.075, 0.004, 0.006), slotMat);
    slot.position.set(p.x, p.y - 0.004, p.z);
    internals.add(slot);
    [-1, 1].forEach((s) => {
      const clip = new THREE.Mesh(new THREE.BoxGeometry(0.005, 0.008, 0.008), clipMat);
      clip.position.set(p.x + s * 0.042, p.y - 0.002, p.z);
      internals.add(clip);
    });
    // Silkscreen slot label printed right on the board, same as a real
    // laptop mainboard identifies DIMM_A / DIMM_B next to each socket.
    const labelTex = canvasTex(64, 20, (ctx, w, h) => {
      ctx.clearRect(0, 0, w, h);
      ctx.fillStyle = "#8a9098";
      ctx.font = "bold 12px Arial";
      ctx.textAlign = "center";
      ctx.fillText(`SLOT ${i + 1}`, w / 2, h * 0.75);
    });
    const label = new THREE.Mesh(
      new THREE.PlaneGeometry(0.028, 0.009),
      new THREE.MeshStandardMaterial({ map: labelTex, transparent: true, roughness: 0.8 })
    );
    label.rotation.x = -Math.PI / 2;
    label.position.set(p.x - 0.05, -0.0038, p.z);
    internals.add(label);
  });
  const slot1Pos = slotPositions[0];

  const ramStick = buildATechRam();
  ramStick.rotation.z = Math.PI / 2;
  ramStick.position.copy(slot1Pos);
  // Non-interactive — this drive is already seated correctly and isn't
  // part of the fault chain for this scenario.
  internals.add(ramStick);

  // -------- M.2 SSD storage — the new second drive + the original boot
  // drive it bumped loose during install --------
  const m2ConnTex = canvasTex(64, 20, (ctx, w, h) => {
    ctx.fillStyle = "#1c1e21";
    ctx.fillRect(0, 0, w, h);
    ctx.fillStyle = "#c9a227";
    for (let x = 4; x < w - 4; x += 3) ctx.fillRect(x, h * 0.3, 1.4, h * 0.4);
  });
  const m2ConnMat = new THREE.MeshStandardMaterial({ map: m2ConnTex, metalness: 0.6, roughness: 0.4 });

  // New M.2 SSD — resting at an angle, not seated, and missing its
  // retaining screw. Two separate fault-fix tasks: press it flush, then
  // drive the screw home (can't screw down a drive still standing proud).
  const newSsdSlotPos = new THREE.Vector3(0.005, -0.0025, -0.02);
  const NEW_SSD_LEN = 0.06;
  const newSsdScrewPos = new THREE.Vector3(newSsdSlotPos.x, newSsdSlotPos.y, newSsdSlotPos.z + NEW_SSD_LEN);

  const newSsdSlotConn = new THREE.Mesh(new THREE.BoxGeometry(0.014, 0.003, 0.008), m2ConnMat);
  newSsdSlotConn.position.copy(newSsdSlotPos);
  internals.add(newSsdSlotConn);

  const newSsdStandoff = new THREE.Mesh(
    new THREE.CylinderGeometry(0.0035, 0.0035, 0.003, 12),
    new THREE.MeshStandardMaterial({ color: 0x0a0b0c, roughness: 0.7 })
  );
  newSsdStandoff.position.copy(newSsdScrewPos);
  newSsdStandoff.userData = { interactable: true, kind: "connector", id: "newSsdScrewed" };
  internals.add(newSsdStandoff);

  const newSsdGroup = new THREE.Group();
  newSsdGroup.position.copy(newSsdSlotPos);
  internals.add(newSsdGroup);
  const newSsdStick = buildM2Stick(["NEW M.2", "SSD 2TB"], NEW_SSD_LEN);
  newSsdStick.position.z = NEW_SSD_LEN / 2;
  newSsdStick.userData = { interactable: true, kind: "connector", id: "newSsdSeated" };
  newSsdGroup.add(newSsdStick);
  newSsdGroup.rotation.x = 0.4; // resting at an angle: far end propped up, not seated

  const newSsdScrew = new THREE.Mesh(
    new THREE.CylinderGeometry(0.0032, 0.0032, 0.005, 12),
    new THREE.MeshStandardMaterial({ color: COLORS.screw, metalness: 0.85, roughness: 0.22 })
  );
  const newSsdScrewHead = new THREE.Mesh(
    new THREE.CircleGeometry(0.0032, 12),
    new THREE.MeshStandardMaterial({ map: screwHeadTex, metalness: 0.8, roughness: 0.3, side: THREE.DoubleSide })
  );
  newSsdScrewHead.rotation.x = Math.PI / 2;
  newSsdScrewHead.position.y = -0.0026;
  newSsdScrew.add(newSsdScrewHead);
  newSsdScrew.position.copy(newSsdScrewPos);
  newSsdScrew.position.y += 0.003;
  newSsdScrew.visible = false;
  internals.add(newSsdScrew);

  // Original boot SSD — already installed and working, but its connector
  // was bumped ajar while the new drive was being fitted next to it. Same
  // ZIF-style hinged-latch mechanic as a display cable: a small flap shown
  // flipped open when disconnected.
  const origSsdPos = new THREE.Vector3(0.035, -0.0025, 0.035);
  ...
  const origSsdConn = new THREE.Mesh(
    new THREE.BoxGeometry(0.016, 0.005, 0.01),
    new THREE.MeshStandardMaterial({ map: connTex, color: COLORS.connectorOff, roughness: 0.5, side: THREE.DoubleSide })
  );
  origSsdConn.position.copy(origSsdPos);
  origSsdConn.userData = { interactable: true, kind: "connector", id: "origSsdReseated" };
  internals.add(origSsdConn);

  const origSsdLatchGroup = new THREE.Group();
  origSsdLatchGroup.position.set(0, 0.0035, -0.006);
  origSsdConn.add(origSsdLatchGroup);
  const origSsdLatchFlap = new THREE.Mesh(
    new THREE.BoxGeometry(0.018, 0.0015, 0.006),
    new THREE.MeshStandardMaterial({ color: 0xc9cdd4, metalness: 0.5, roughness: 0.4 })
  );
  origSsdLatchFlap.position.z = 0.003;
  origSsdLatchGroup.add(origSsdLatchFlap);
  origSsdLatchGroup.rotation.x = -1.05; // disconnected: flap flipped open

  const origSsdStick = buildM2Stick(["BOOT SSD", "1TB"], 0.05);
  origSsdStick.position.set(origSsdPos.x, origSsdPos.y + 0.003, origSsdPos.z - 0.03);
  internals.add(origSsdStick);
  ...
  function renderTaskList() {
    ...
    const html = `<div class="tl-title">SSD Upgrade Repair</div><ul>${TASKS.map((t) => {
  ...
  function updateHint() {
    ...
    if (kind === "plug" && !taskState.unplug.done) text = `<kbd>E</kbd> Unplug AC adapter from wall outlet`;
    ...
    else if (kind === "manual") text = `<kbd>E</kbd> Read storage configuration manual`;
      ...
      if (id === "newSsdSeated" && !taskState.newSsdSeated.done) text = `<kbd>Click</kbd> Press the new M.2 SSD flush into its slot`;
      else if (id === "newSsdScrewed" && !taskState.newSsdScrewed.done) {
        text = taskState.newSsdSeated.done ? `<kbd>Click</kbd> Install the retaining screw` : `Seat the drive flush before screwing it down`;
      } else if (id === "origSsdReseated" && !taskState.origSsdReseated.done) text = `<kbd>Click</kbd> Reseat the original SSD's connector`;
    }
  ...
  function repairsReady() {
    return taskState.newSsdSeated.done && taskState.newSsdScrewed.done && taskState.origSsdReseated.done;
  }
  ...
  function fixConnector(id) {
    if (id === "newSsdSeated") {
      let t = 0;
      const startRot = newSsdGroup.rotation.x;
      const anim = () => {
        t += 0.12;
        newSsdGroup.rotation.x = startRot * (1 - Math.min(1, t));
        if (t < 1) requestAnimationFrame(anim);
        else newSsdGroup.rotation.x = 0;
      };
      anim();
      markDone("newSsdSeated");
      toast("New M.2 SSD pressed flush into its slot");
    } else if (id === "newSsdScrewed") {
      newSsdScrew.visible = true;
      newSsdScrew.scale.set(1, 0.01, 1);
      let t = 0;
      const anim = () => {
        t += 0.12;
        newSsdScrew.scale.y = Math.min(1, t);
        if (t < 1) requestAnimationFrame(anim);
      };
      anim();
      markDone("newSsdScrewed");
      toast("Retaining screw installed");
    } else if (id === "origSsdReseated") {
      origSsdConn.material.color.setHex(COLORS.connectorOn);
      let t = 0;
      const startRot = origSsdLatchGroup.rotation.x;
      const anim = () => {
        t += 0.1;
        origSsdLatchGroup.rotation.x = startRot * (1 - Math.min(1, t));
        if (t < 1) requestAnimationFrame(anim);
        else origSsdLatchGroup.rotation.x = 0;
      };
      anim();
      markDone("origSsdReseated");
      toast("Original boot SSD's connector reseated");
    }
  }
  ...
  function runTest() {
    ...
    const iv = setInterval(() => {
      ...
      if (blinks > 5) {
        ...
        if (success) {
          toast("Windows booted successfully");
  ...
  function openManualModal() {
    const wrap = el("div");
    wrap.innerHTML = `<div class="mc-kicker">Reference</div><h2>${STORAGE_SPEC_SHEET.title}</h2>`;
    const table = el("table", "spec-table");
    STORAGE_SPEC_SHEET.rows.forEach(([label, value]) => {
      const tr = el("tr");
  ...
  function openDiagnoseModal() {
    ...
    wrap.innerHTML =
      ...
      `<p class="mc-body">Symptom: powers on and POSTs normally, but stops before Windows loads, since a second M.2 SSD was installed. Select every issue you can see.</p>`;
  ...
  function tryPrimaryAction() {
    ...
    if (kind === "screw" && !removedScrews.has(id)) {
      ...
      // Can't drive the retaining screw home while the drive is still
      // standing proud of its slot — seat it flush first.
      if (id === "newSsdScrewed" && !taskState.newSsdSeated.done) return;
      fixConnector(id);
