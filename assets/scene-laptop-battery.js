...
const BATTERY_SPEC_SHEET = {
  title: "Razer Blade 18 — Battery Service Notes",
  rows: [
    ["Model", "Razer Blade 18 (2026), 99.8Wh Li-ion battery, single internal connector"],
    ["Battery connector", "Single keyed header — must be pressed straight down until it clicks; a half-seated connector reads as no power source at all"],
    ["Keyboard ribbon cable", "Power button is wired through the keyboard controller on this model — a disconnected keyboard ribbon can prevent the power-on signal from ever reaching the board"],
    ["Screw care during battery service", "Bottom-panel screws are different lengths from the battery bracket screws — never let a loose screw fall onto the board; a stray screw across the wrong contacts can short the power circuit"],
    ["ESD note", "Battery swaps are one of the most common self-repair jobs customers attempt — always re-check strap, mat, and connector seating before assuming a deeper fault"],
  ],
};

const DIAGNOSE_OPTIONS = [
  {
    id: "batteryConn", correct: true,
    label: "Battery connector isn't seated",
    explain: "Correct — with the battery connector unplugged, the system has zero power source once the AC adapter is removed, and can't even attempt to boot.",
  },
  {
    id: "kbRibbon", correct: true,
    label: "Keyboard ribbon cable is disconnected",
    explain: "Correct — on this model the power button routes through the keyboard controller; a disconnected ribbon can prevent the power-on signal from ever reaching the board.",
  },
  {
    id: "strayScrew", correct: true,
    label: "A stray screw is shorting the board",
    explain: "Correct — a loose conductive screw resting across the wrong contacts can short out the power circuit entirely.",
  },
  {
    id: "charger", correct: false,
    label: "AC adapter/charger is dead",
    explain: "Not the cause here — the timing (right after a battery swap, with the internals disturbed) points to something inside, not the external adapter.",
  },
  {
    id: "boardFail", correct: false,
    label: "Motherboard power circuit has failed outright",
    explain: "Not the cause here — a fully failed board wouldn't be explained by what changed (a battery swap); check what was actually disturbed first.",
  },
];
...
const TASKS = [
  ...
  { id: "batteryConn", label: "Reseat the battery connector." },
  { id: "kbRibbon", label: "Reconnect the keyboard ribbon cable." },
  { id: "strayScrew", label: "Remove the stray screw shorting the board." },
...
export async function buildFPScene(container, { hintEl, toastEl, crosshairEl, taskListEl, wornItemsEl, modalOverlayEl, modalCardEl }) {
  ...
  const ticketTex = canvasTex(280, 360, (ctx, w, h) => {
    ...
    ctx.fillText("Battery replaced by customer", 16, 82);

    ctx.font = "bold 10px Arial";
    ctx.fillText("REPORTED SYMPTOM", 16, 100);
    ctx.font = "10px Arial";
    const lines = [
      "Completely dead — no fan, no",
      "LED, no response to the power",
      "button — since a customer",
      "battery replacement.",
    ];
    ...
    const checks = [
      "Battery connector seated", "Keyboard ribbon cable", "Stray screws",
      "AC adapter/charger", "Motherboard power circuit",
    ];
    checks.forEach((c, i) => {
      const y = 216 + i * 20;
  ...
  // Two SO-DIMM slots — decorative only in this scenario (RAM isn't the
  // fault here); Slot 1 is populated so the board doesn't look stripped,
  // Slot 2 is left empty exactly like a single-stick config would be.
  ...
  internals.add(ramStick); // decorative, not interactable — RAM isn't this scenario's fault

  // -------- Keyboard ribbon cable connector (fault #1) --------
  // A flat FPC-style ribbon into a ZIF (zero-insertion-force) socket with a
  // small hinged locking flap, exactly how a real keyboard ribbon clips in.
  // On this model the power button is wired through the keyboard
  // controller, so a disconnected ribbon can block the power-on signal.
  ...
  const kbRibbonTex = canvasTex(96, 24, (ctx, w, h) => {
    ctx.fillStyle = "#d9a233";
    ctx.fillRect(0, 0, w, h);
    ctx.strokeStyle = "rgba(0,0,0,0.3)";
    ctx.lineWidth = 1;
    for (let x = 2; x < w; x += 4) { ctx.beginPath(); ctx.moveTo(x, 0); ctx.lineTo(x, h); ctx.stroke(); }
  });
  const kbRibbonGroup = new THREE.Group();
  const kbRibbonStrip = new THREE.Mesh(
    new THREE.BoxGeometry(0.03, 0.0012, 0.01),
    new THREE.MeshStandardMaterial({ map: kbRibbonTex, roughness: 0.6, side: THREE.DoubleSide })
  );
  kbRibbonStrip.position.z = 0.012;
  kbRibbonGroup.add(kbRibbonStrip);
  const kbRibbonConn = new THREE.Mesh(
    new THREE.BoxGeometry(0.018, 0.006, 0.012),
    new THREE.MeshStandardMaterial({ map: connTex, color: COLORS.connectorOff, roughness: 0.5, side: THREE.DoubleSide })
  );
  kbRibbonGroup.add(kbRibbonConn);
  kbRibbonGroup.position.set(0.02, -0.003, LD * 0.4);
  kbRibbonConn.userData = { interactable: true, kind: "connector", id: "kbRibbon" };
  internals.add(kbRibbonGroup);

  const kbLatchGroup = new THREE.Group();
  kbLatchGroup.position.set(0, 0.003, -0.006);
  kbRibbonConn.add(kbLatchGroup);
  const kbLatchFlap = new THREE.Mesh(
    new THREE.BoxGeometry(0.02, 0.0015, 0.006),
    new THREE.MeshStandardMaterial({ color: 0xc9cdd4, metalness: 0.5, roughness: 0.4 })
  );
  kbLatchFlap.position.z = 0.003;
  kbLatchGroup.add(kbLatchFlap);
  kbLatchGroup.rotation.x = -1.1; // disconnected: flap flipped open

  // -------- Stray screw shorting the board (fault #2) --------
  // The same screw model used on the bottom panel, left lying flat across
  // a component near the vapor chamber — exactly the kind of thing a
  // rushed self-repair battery swap leaves behind.
  const strayScrewMesh = new THREE.Mesh(
    new THREE.CylinderGeometry(0.007, 0.007, 0.012, 16),
    new THREE.MeshStandardMaterial({ color: COLORS.screw, metalness: 0.85, roughness: 0.22 })
  );
  const strayScrewHead = new THREE.Mesh(
    new THREE.CircleGeometry(0.007, 16),
    new THREE.MeshStandardMaterial({ map: screwHeadTex, metalness: 0.8, roughness: 0.3, side: THREE.DoubleSide })
  );
  strayScrewHead.rotation.x = Math.PI / 2;
  strayScrewHead.position.y = -0.0061;
  strayScrewMesh.add(strayScrewHead);
  strayScrewMesh.rotation.x = Math.PI / 2; // lying flat, resting across a component
  strayScrewMesh.position.set(-LW * 0.1, -0.0065, LD * 0.21);
  strayScrewMesh.userData = { interactable: true, kind: "connector", id: "strayScrew" };
  internals.add(strayScrewMesh);

  // -------- Battery connector (fault #3) --------
  // The battery's power lead, unplugged and dangling just short of its
  // socket on the board — a half-seated or fully-unplugged battery
  // connector is the single most common mistake in a DIY battery swap.
  const batterySocketPos = new THREE.Vector3(0.045, -0.003, 0.02);
  const batterySocket = new THREE.Mesh(
    new THREE.BoxGeometry(0.014, 0.006, 0.02),
    new THREE.MeshStandardMaterial({ color: 0x1c1e21, roughness: 0.6 })
  );
  batterySocket.position.copy(batterySocketPos);
  internals.add(batterySocket);

  // Fixed lead from the battery pack to just short of the socket — the
  // last centimeter is the animated plug itself.
  const batteryLeadCurve = new THREE.CatmullRomCurve3([
    new THREE.Vector3(0.09, -0.001, 0.05),
    new THREE.Vector3(0.072, 0.005, 0.038),
    new THREE.Vector3(0.06, 0.009, 0.028),
  ]);
  internals.add(new THREE.Mesh(
    new THREE.TubeGeometry(batteryLeadCurve, 12, 0.0025, 6, false),
    new THREE.MeshStandardMaterial({ color: 0xb91c1c, roughness: 0.7 })
  ));

  const batteryPlugSeatPos = new THREE.Vector3(0.045, -0.0007, 0.02);
  const batteryPlugDanglePos = new THREE.Vector3(0.058, 0.011, 0.028);
  const batteryPlugDangleRotZ = 0.55;
  const batteryPlug = new THREE.Mesh(
    new THREE.BoxGeometry(0.012, 0.006, 0.016),
    new THREE.MeshStandardMaterial({ color: COLORS.connectorOff, roughness: 0.5 })
  );
  batteryPlug.position.copy(batteryPlugDanglePos);
  batteryPlug.rotation.z = batteryPlugDangleRotZ;
  batteryPlug.userData = { interactable: true, kind: "connector", id: "batteryConn" };
  internals.add(batteryPlug);
  ...
  function renderTaskList() {
    ...
    const html = `<div class="tl-title">Battery Swap Gone Wrong</div><ul>${TASKS.map((t) => {
  ...
  function updateHint() {
    ...
    if (kind === "plug" && !taskState.unplug.done) text = `<kbd>E</kbd> Unplug AC adapter from wall outlet`;
    ...
    else if (kind === "manual") text = `<kbd>E</kbd> Read battery service manual`;
      ...
      if (id === "batteryConn" && !taskState.batteryConn.done) text = `<kbd>Click</kbd> Reseat battery connector`;
      if (id === "kbRibbon" && !taskState.kbRibbon.done) text = `<kbd>Click</kbd> Reconnect keyboard ribbon cable`;
      if (id === "strayScrew" && !taskState.strayScrew.done) text = `<kbd>Click</kbd> Remove stray screw`;
    }
  ...
  function repairsReady() {
    return taskState.batteryConn.done && taskState.kbRibbon.done && taskState.strayScrew.done;
  }
  ...
  function fixConnector(id) {
    if (id === "kbRibbon") {
      kbRibbonConn.material.color.setHex(COLORS.connectorOn);
      kbRibbonConn.position.y += 0.004;
      let t = 0;
      const startRot = kbLatchGroup.rotation.x;
      const anim = () => {
        t += 0.1;
        kbLatchGroup.rotation.x = startRot * (1 - Math.min(1, t));
        if (t < 1) requestAnimationFrame(anim);
        else kbLatchGroup.rotation.x = 0;
      };
      anim();
      markDone("kbRibbon");
      toast("Keyboard ribbon cable reconnected");
    } else if (id === "strayScrew") {
      let t = 0;
      const anim = () => {
        t += 0.1;
        strayScrewMesh.scale.setScalar(Math.max(0, 1 - t));
        if (t < 1) requestAnimationFrame(anim);
        else strayScrewMesh.visible = false;
      };
      anim();
      markDone("strayScrew");
      toast("Stray screw removed");
    } else if (id === "batteryConn") {
      let t = 0;
      const startPos = batteryPlug.position.clone();
      const startRotZ = batteryPlug.rotation.z;
      const anim = () => {
        t += 0.12;
        const tt = Math.min(1, t);
        batteryPlug.position.lerpVectors(startPos, batteryPlugSeatPos, tt);
        batteryPlug.rotation.z = startRotZ * (1 - tt);
        if (t < 1) requestAnimationFrame(anim);
        else {
          batteryPlug.position.copy(batteryPlugSeatPos);
          batteryPlug.rotation.z = 0;
          batteryPlug.material.color.setHex(COLORS.connectorOn);
        }
      };
      anim();
      markDone("batteryConn");
      toast("Battery connector reseated");
    }
  }
  ...
  function runTest() {
    ...
    const iv = setInterval(() => {
      ...
      if (blinks > 5) {
        ...
        if (success) {
          toast("Fan spins, LED lit — POST successful");
  ...
  function openManualModal() {
    const wrap = el("div");
    wrap.innerHTML = `<div class="mc-kicker">Reference</div><h2>${BATTERY_SPEC_SHEET.title}</h2>`;
    const table = el("table", "spec-table");
    BATTERY_SPEC_SHEET.rows.forEach(([label, value]) => {
      const tr = el("tr");
  ...
  function openDiagnoseModal() {
    ...
    wrap.innerHTML =
      ...
      `<p class="mc-body">Symptom: completely dead since a customer battery replacement &mdash; no fan, no LED, no response to the power button. Select every issue you can see.</p>`;
